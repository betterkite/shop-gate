...
description: Fetch and validate retail behavior and product-window data through Shop Gate's local-first commerce-data backend. Use for window coverage, behavior/category metrics, coverage, or evidence needed by retail dashboards.
---
...
1. 使用 `commerce-entity-resolver` 确认代码和资产类型；使用 `commerce-data-registry` 确认本地覆盖。
