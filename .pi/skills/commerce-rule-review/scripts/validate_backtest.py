#!/usr/bin/env python3
"""Validate a reproducible long-only Shop Gate backtest result."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate a Shop Gate long-only backtest result JSON.")
    parser.add_argument("--input", default="-", help="JSON file path, or '-' for stdin (default).")
    args = parser.parse_args()
    try:
        payload = load_json(args.input)
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        return emit({"ok": False, "contract": "commerce-rule-review/v1", "errors": [str(exc)], "warnings": []}, 2)
    errors, warnings, stats = validate(payload)
    return emit({"ok": not errors, "contract": "commerce-rule-review/v1", "errors": errors, "warnings": warnings, "stats": stats}, 1 if errors else 0)
