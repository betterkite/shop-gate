...
25. `.data-agent/**` 全部属于平台只读产物，包括 workspace/profile/task/plan、Finance Query Rewrite/Run Plan、generation state/queue、events、validation、repair plan、artifact contracts 和视觉报告。只能读取并据此修改 `app/**`、final 数据或 evidence，禁止编辑、删除、追加或伪造任何 `.data-agent` 文件。
...
3. 上述验证报告以及 `.data-agent/artifact-contracts.json`、`.data-agent/generation-state.json`、`.data-agent/generation-queue.json` 均为平台只读产物。不得编辑、删除或伪造 viewport、截图、passed 状态或验证结果；修复完页面、final 数据或 evidence 后等待平台重新验证。
