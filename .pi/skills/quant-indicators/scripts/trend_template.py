...
def main() -> int:
    parser = argparse.ArgumentParser(description="生成 Shop Gate 趋势模板摘要。")
    ...
    try:
        emit(build_trend_template(read_json(args.input)), args.output)
    except (OSError, UnicodeError, json.JSONDecodeError, ValueError) as error:
        print(f"trend_template: {error}", file=sys.stderr)
        return 2
    return 0
