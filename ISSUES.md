...
| ISSUE-P28 | 电商经营分析能力扩展 | P17 | 用户漏斗与留存、RFM、渠道/活动归因、毛利/利润、补货预测、商品生命周期和价格弹性形成可运行能力；每项能力有数据缺口提示、看板模板、后端聚合、E2E case 和桌面/移动验收 | in-progress（已完成 RFM、渠道/活动、毛利、库存健康、商品经营阶段、价格带边界 7 个后端接口；新增 `/analytics-workbench` BI 工作台、Agent 下钻接口/allowlist 和 `analytics-bi` 生成模板第一阶段；商品阶段支持全量分页、四阶段筛选和规则说明；P28 R23 真实生成任务单例 E2E 已通过，包含 run plan、扩展数据预取、页面 build、预览 HTTP 200、数据证据、产物契约和桌面/移动视觉验收；仍需真实聊天多轮 E2E，价格弹性继续明确数据不足；数据集导入联动另见 ISSUE-P31；证据见 [`docs/review/p28-analytics-capabilities.md`](docs/review/p28-analytics-capabilities.md)） |
...
| ISSUE-P30 | 电商 Agent 任务闭环与 BI 交互增强 | P17 | 支持自然语言多轮下钻、商品/类目/渠道筛选、图表联动、解释口径、行动建议和用户可理解的限制说明；不把缺失数据伪装成结论；新增真实零售 E2E 和视觉验收 | open（承接金融 Agent 的多维比较、证据追踪、风险诊断和策略解释能力，但不恢复金融领域代码；本轮已修复“只做问答仍生成看板”和生成完成后对话视图被轮询切回看板，仍需 DeepSeek 真实多轮回归验收） |
