...
| ISSUE-P8 | Skills 清洗与功能级电商化 | P8 | 12 个核心 Skill 的命名、registry、lock、tgz、capsule 一致；6 个领域 Skill 的脚本按电商指标工作 | done（P29 已完成六个领域 Skill 的 body/scripts、registry、capsule、lock、tgz 和 1.1.0 不可变快照同步；金融计算逻辑和旧参考合同已移除，门禁通过） |
| ISSUE-P9 | 零售评测与 E2E 基准重建 | P9 | 30 条零售 case、4 个能力、真实运行证据、评测报告和零售语料门禁完整 | done（P29 已完成 task-E2E 默认零售数据集、triad Query Rewrite、mutation/replay 快照和评测门禁重建；`check:eval-datasets`、benchmark coverage、judge calibration、mutation 和 54 个 eval 单测通过） |
...
| [ISSUE-P17](https://github.com/betterkite/shop-gate/issues/14) | 电商经营分析体验与生成看板视觉升级 | P17 | 商品运营/经营情报有统一返回入口；桌面与移动端视觉通过；生成看板形成“总览→趋势→异常→拆解→行动”闭环，所有金额/库存来源显式标注；模板校验、类型检查与 retail e2e 通过 | in-progress（本切片按可访问的相关电商 BI 方案建立指标与分析路径；用户提供的知乎原文直连返回 403、浏览器加载超时，未将其当作已核验内容；本轮新增 UV 去重口径、中文优先指标标签及移动端溢出修复） |
| [ISSUE-P18](https://github.com/betterkite/shop-gate/issues/22) | 零售看板数据分析深度：用户漏斗、类目诊断与日报异动 | P17 | 漏斗同时展示事件数与独立用户触达率；类目看板提供高流量低转化诊断；日报异动榜按末日环比绝对值排序并给出分析建议；数据契约、后端测试、模板验证和 retail e2e 通过 | done（实现、恢复链路修复、单测和 DeepSeek 运行态 R01/R02/R04 3/3 READY 完成，待用户业务验收；记录见 `docs/review/p18-retail-analytics.md`） |
| [ISSUE-P19](https://github.com/betterkite/shop-gate/issues/23) | 看板任务闭环：成果入口、Query Rewrite 鉴权诊断与库销比流量转化 | P17 | 成果入口回到最近成果；DeepSeek 401/403 或模型不存在时给出可操作错误；价库看板展示库销比最差 Top 10、页面浏览量（PV）、购买转化率（Buy / PV）；数据契约、模板、类型与运行态验收通过 | done（实现、单测、构建和运行态预览已通过；有效 DeepSeek 直连下用户原问句与 R15 价格/库存场景均 `READY`、预览 HTTP 200；补充修复 Top-N 库销比问题被误要求指定比较范围，证据见 `docs/review/p19-dashboard-journey.md`） |
...
| [ISSUE-P22](https://github.com/betterkite/shop-gate/issues/24) | 合成行为漏斗一致性：购买不能脱离页面浏览 | P20 | 合成行为的商品级/日级 `Fav/Cart/Buy` 均不超过对应 `PV`；全局漏斗、商品池、类目和渠道口径通过回归测试与数据重建验收 | done（商品/类目/渠道/日趋势全量扫描 0 例 `buy > pv`；合成数据测试 14 passed；release gate 全绿；fresh Agent E2E 仍需本地登录凭据） |
...
| [ISSUE-P28](https://github.com/betterkite/shop-gate/issues/17) | 电商经营分析能力扩展 | P17 | 用户漏斗与留存、RFM、渠道/活动归因、毛利/利润、补货预测、商品生命周期和价格弹性形成可运行能力；每项能力有数据缺口提示、看板模板、后端聚合、E2E case 和桌面/移动验收 | in-progress（已完成 RFM、渠道/活动、毛利、库存健康、补货参考、商品经营阶段、用户留存、价格带与价格弹性边界 9 个后端接口；新增 `/analytics-workbench` BI 工作台、Agent 下钻接口/allowlist 和 `analytics-bi` 生成模板第一阶段；商品阶段支持全量分页、四阶段筛选和规则说明，用户留存支持首购周 cohort、完整 7 日观察窗口和 7 日留存率，补货参考支持全量分页、显式供货假设、优先级和原因说明，并接入生成式 BI 的预取、数据契约和看板模板；P28 R23 真实生成任务单例 E2E 已通过，`p28-experiment` 重跑同样 `ready=1/1`、校验通过且预览 HTTP 200；真实 DeepSeek 聊天单轮与第二轮追问均已确认 `answer_ready`、durable job `completed` 且无预览；本轮新增 `dataset_price_experiment_observations`、用户级 outcomes 和合成对照/处理组模拟，后端、工作台和生成模板按 `estimated`/`synthetic_experiment_reference`/`aggregate_reference_only`/`ready_for_user_level_review` 状态展示观察关系、实验模拟或证据缺口；P28 Issue #52 已补充用户级统计复核，Issue #54 正在补充多实验 FDR 校正边界；正式因果模型和分层检验仍未完成，数据集导入联动另见 ISSUE-P31；证据见 [`docs/review/p28-analytics-capabilities.md`](docs/review/p28-analytics-capabilities.md)） |
...
| [ISSUE-P30](https://github.com/betterkite/shop-gate/issues/15) | 电商 Agent 任务闭环与 BI 交互增强 | P17 | 支持自然语言多轮下钻、商品/类目/渠道筛选、图表联动、解释口径、行动建议和用户可理解的限制说明；不把缺失数据伪装成结论；新增真实零售 E2E 和视觉验收 | in-progress（承接金融 Agent 的多维比较、证据追踪、风险诊断和策略解释能力，但不恢复金融领域代码；已修复“只做问答仍生成看板”、生成完成后对话视图被轮询切回看板、问答模式显示 `dashboard-visualization`、跨能力全库上下文丢失、首页默认漏斗覆盖库存问答、Agent Runtime 5 秒事务超时、问答 durable job 保持 `running` 以及 answer-ready 后前端仍可继续排队等问题；真实 DeepSeek 单轮与第二轮追问已通过；本次补齐商品、类目和渠道下钻入口，返回 PV、收藏、加购、购买、购买转化证据、建议动作及数据集/维度/值上下文链接，并完成 API/前端浏览器验收；普通指代型追问现在可继承上一轮实体、数据集和时间范围，明确新实体会覆盖旧上下文，并有回归测试；新增日趋势接口和工作台筛选联动，渠道/活动未采集行为归因时不复用全店 PV，并完成真实 API/浏览器验收；本轮继续完成总览、用户分群、渠道/活动、利润以及商品/类目适用的库存、商品阶段和价格观察多模块筛选联动，每个接口返回 `scope`，不适用的渠道/活动商品级分析明确保留全店口径并说明原因；本轮修复商品/类目问答预取遗漏 `dataset_id` 导致读取旧数据集的问题，最终数据增加数据集身份校验；只读问答不强行修改 provider 工具选择协议，若 provider 输出纯文本则通过纯 `submit_result` 兜底收敛候选；趋势接口和预取层现保留可点击的 `context_url`，总览筛选会同步展示当前范围摘要并刷新趋势；run plan 现记录可审计多轮 context，并将数据集、实体、时间范围和已验证的筛选维度和值显式注入 Agent 任务合同；本轮修复聊天问题文本中明确数据集未绑定请求而回退默认全店数据的问题，并修复继承实体未同步到 `plannedEntities` 导致第二轮重新取全店数据的问题；真实 DeepSeek 两轮验收已确认第一轮 PV `376`/购买 `19`/转化率 `5.05%`，第二轮指代追问 PV `84`/购买 `5`/转化率 `5.95%`，两轮均无看板生成；新增 `check:analytics-workbench-visual` 并通过 1440×900 桌面与 390×844 移动视觉验收；本轮补齐总览商品阶段卡片的阶段/范围联动，分析页翻页保留商品/类目筛选，毛利渠道表新增渠道明细入口，并纳入桌面/移动视觉回归；本轮再补齐只读回答的上下文收口：平台从权威 final 数据自动补充数据集、筛选范围、时间窗口和可打开分析链接，避免依赖模型主动写链接；已有链接不重复追加；本轮补齐用户分群卡片按具体分群过滤，后端分类规则不变并返回当前分群人数/分页；实时数据集验收确认“新近购买”61 个用户、4 页；本轮补齐总览渠道摘要的“查看全部渠道和活动”出口，并保留当前筛选范围；本轮补齐趋势图日期级联动，日期可进入受数据集窗口约束的“日趋势明细”，并通过后端单测、真实 API 和桌面/移动视觉回归；仍需补齐更完整的图表联动，记录见 [`docs/review/p30-agent-bi-interaction.md`](docs/review/p30-agent-bi-interaction.md)） |
| [ISSUE-P31](https://github.com/betterkite/shop-gate/issues/16) | 电商数据集选择与导入后分析联动 | P17 | BI 工作台提供数据集选择器，展示窗口、来源、行数和合成字段；切换数据集后所有分析视图同步读取目标 `dataset_id`；数据集导入完成后自动刷新可用数据集与当前分析，按配置异步触发看板生成；刷新/生成结果有状态、失败原因和证据记录，不重复生成、不覆盖用户正在查看的其他数据集 | in-progress（已完成选择器、自动刷新、受控合成数据集导入、最近任务历史、标准行为事件 CSV Web 上传、`dataset_id` 隔离的行为明细持久化、商品级行为接口、导入后 `analytics-bi` 配置任务，并修复 Agent 预取硬编码旧数据集的问题：工作台选择会写入当前浏览器上下文，聊天任务将该 `datasetId` 固化到 run plan 和各扩展分析请求；CSV 之外的第三方连接器、配置清单到 Agent 持久看板预览的跨服务编排仍待后续片，验收记录见 [`docs/review/p31-dataset-linkage.md`](docs/review/p31-dataset-linkage.md)） |
| [ISSUE-P32](https://github.com/betterkite/shop-gate/issues/18) | 导入任务到 Agent 看板的跨服务持久化编排 | P17 | 数据集导入/更新后，在明确的 `project_id` 范围内消费 `analytics-bi` 配置清单；任务具备 outbox/幂等键、失败重试和证据链；Agent 生成并验证持久预览，任务中心可打开正确数据集且不覆盖其他项目或数据集 | done（PR #39 已合并；RepoSteward follow-up 已确认项目范围、幂等/outbox、Agent 消费、持久预览和任务中心四组 GitHub checks 全部通过；Issue #18 已关闭） |
...
P30 续更：补货摘要四类判断已分别关联到对应的商品清单，接口新增 `priority` 筛选并保留全量优先级统计、商品/类目范围和分页；供货周期、目标覆盖天数和参考边界仍完整展示。后端单测、API 验收与桌面/移动视觉回归已通过。

P30 续更：价格带表与筛选入口已关联到同一 `price_band`，商品价格观察、实验参考和商品/类目范围会同步更新；数据不足时继续明确展示缺口，不把观察关系当成实验结论。后端单测、API 验收与桌面/移动视觉回归已通过。

P30 续更：商品或类目筛选下进入渠道/活动明细时，接口与日趋势新增 `filter_dimension` + `filter_value` 复合范围，结果、上下文链接和页面父级范围说明保持一致；后端单测、真实 API 验收、TypeScript/Lint 及桌面/移动视觉回归均已通过。

P30 续更：主页、问答输入提示和当前生成看板模板已统一用户可见术语，将“漏斗”改为“浏览到购买的转化过程”，将“Resolver/标的”等内部表达改为“检查商品/类目名称/保留问题中的名称”，并将示例中的 GMV 改为“成交总额（GMV）”；内部 `traffic_funnel`、`funnel` 接口和模板 ID 保持兼容。全量单测、类型检查、Lint、脚手架模板构建、主页和看板桌面/移动视觉验收均已通过。

P30 续更：价格弹性页的商品观察行新增商品明细入口，用户可从价格参考继续核对页面浏览、购买、库存和利润；视觉门新增该入口检查。

P30 续更：多轮追问现在可以继承上一轮结果中的商品集合；例如“库销比最差 10 个商品”后追问“这些商品的流量转化如何”，仅在运行 ID 与数据集身份一致时写入 `plannedEntities.itemIds`，并有回归测试保护，避免错误回退全店口径。

P30 续更：经营分析总览新增用户留存和补货参考摘要卡，显示核心数字并可携带当前商品/类目范围进入完整分析页；桌面/移动视觉门新增两个入口及范围保留检查。

P30 续更：趋势图日期现可进入“日趋势明细”；commerce-data 的趋势接口新增受数据集窗口约束的 `start`/`end`，页面按所选日期重新统计页面浏览量、购买行为和订单数，并提供返回上一级入口。后端单测、真实 API、TypeScript/Lint 及桌面/移动视觉回归均已通过。

P30 续更：日趋势与筛选摘要现在同时展示页面浏览量（PV）、加购行为（Cart）、购买行为（Buy）和订单数，右侧数量带字段名；每个日期的整行可进入当天明细，并通过桌面/移动视觉回归验证。

P30 续更：商品/类目筛选进入各分析页后统一显示当前范围说明和清除入口，覆盖用户分群、留存、渠道活动、利润、库存、补货、商品阶段和价格观察，避免跨页查看时误读为全店结果。
...
P28 续更：价格实验结果新增购买率变化的 95% 置信区间、两比例近似检验 p 值、每组最低样本量判断和中文解释；工作台与生成式 BI 模板同步展示“样本显示有差异 / 暂未发现明确差异 / 样本较少，暂不判断”。这些统计字段只描述样本差异，仍不替代随机分组证据或正式因果分析。

P28 后续 Issue #44：新增 `dataset_price_experiment_assignments` 用户级分组证据契约；合成数据生成确定性分组清单，质量扫描检查来源、对照/处理覆盖和完整性，价格实验结果按“已记录分组（演示）/未确认随机分组/分组记录不完整/未提供用户分组”区分边界。真实实验文件或平台接入、正式因果模型仍未完成。
...
P28 后续 Issue #52：在完整真实 outcomes 上输出对照组/处理组的用户数、购买用户数、购买件数、购买率、差异区间和 p 值，统计字段只用于样本复核；多个实验明确标记 p 值尚未做多重检验校正，缺失或不完整 outcomes 保持数据缺口状态。正式因果模型、分层检验和校正方法仍留待后续切片。已由 PR #53 合并。

P28 后续 Issue #54：对多个完整用户级实验的 p 值执行 Benjamini-Hochberg FDR 校正，在结果中返回校正方法、阈值和校正后 p 值；完整结果不足时保持未就绪状态。FDR 校正只处理多重比较的统计边界，不等于随机分组证明或因果结论。已由 PR #55 合并。

P28 后续 Issue #56：利用已有用户画像字段按会员等级、城市层级或年龄段输出用户级实验的描述性分层统计；无完整 outcomes、缺失画像或分组不齐时保持明确的数据缺口状态。正式分层因果模型、协变量调整和交互效应检验仍留待后续。

P28 后续 Issue #58：增加可选用户分配平衡检查，按会员等级、城市层级或年龄段比较对照/处理人群构成，显式保留未知画像并区分记录不完整、变体不足和未提供分组。该检查只提供描述性证据，不证明随机化或因果关系；正式协变量调整、异质性效应和因果模型仍留待后续。

P28 后续 Issue #60：为每个价格实验固定用户级分析口径，明确对照/处理组、购买结果、差异方向、结果窗口和未做协变量调整；完整真实证据进入用户级复核状态，合成或缺失证据保持描述性/未就绪边界。正式协变量调整、异质性效应和因果模型仍留待后续。

P28 后续 Issue #62：增加可选分层标准化购买率参考，使用两组用户合并后的画像分布作为权重，返回各层权重和标准化差异；未知画像、变体覆盖不足、用户结果不完整时保留数据缺口，不把描述性标准化结果解释为正式因果调整。倾向得分、回归调整、双重稳健估计和正式因果模型仍留待后续。
