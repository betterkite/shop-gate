...
| ISSUE-P28 | 电商经营分析能力扩展 | P17 | 用户漏斗与留存、RFM、渠道/活动归因、毛利/利润、补货预测、商品生命周期和价格弹性形成可运行能力；每项能力有数据缺口提示、看板模板、后端聚合、E2E case 和桌面/移动验收 | in-progress（已完成 RFM、渠道/活动、毛利、库存健康、商品经营阶段、价格带边界 7 个后端接口；新增 `/analytics-workbench` BI 工作台、Agent 下钻接口/allowlist 和 `analytics-bi` 生成模板第一阶段；商品阶段支持全量分页、四阶段筛选和规则说明；P28 R23 真实生成任务单例 E2E 已通过；真实 DeepSeek 聊天单轮与第二轮追问均已确认 `answer_ready`、durable job `completed` 且无预览；价格弹性继续明确数据不足；数据集导入联动另见 ISSUE-P31；证据见 [`docs/review/p28-analytics-capabilities.md`](docs/review/p28-analytics-capabilities.md)） |
...
| ISSUE-P31 | 电商数据集选择与导入后分析联动 | P17 | BI 工作台提供数据集选择器，展示窗口、来源、行数和合成字段；切换数据集后所有分析视图同步读取目标 `dataset_id`；数据集导入完成后自动刷新可用数据集与当前分析，按配置异步触发看板生成；刷新/生成结果有状态、失败原因和证据记录，不重复生成、不覆盖用户正在查看的其他数据集 | in-progress（已完成选择器和自动刷新第一片：BI 工作台读取 `/api/v1/commerce/datasets`，显示窗口/来源/契约版本/记录数/合成字段，切换后按 `dataset_id` 重新读取当前视图；客户端每 15 秒检测契约变化并自动刷新当前分析；Web 导入入口、质量扫描触发、异步看板生成和商品级行为明细仍待后续片，验收记录见 [`docs/review/p31-dataset-linkage.md`](docs/review/p31-dataset-linkage.md)） |
