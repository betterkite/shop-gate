...
| ISSUE-P27 | 电商数据契约与数据集扩展 | P17 | 在保留现有真实行为数据的基础上，补充可追溯的用户、会话、渠道、活动、成本、库存快照和订单字段；真实与合成边界、生成规则和限制可审计；现有 4 项能力回归不退化 | open（当前数据只能覆盖漏斗、类目、价格库存和日报；先设计追加式 schema/API 与约 1,000 商品经营数据集，再接 Agent） |
