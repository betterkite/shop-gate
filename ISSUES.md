...
| ISSUE-P14 | generation/eval/scaffold 内部历史命名与模板语义零售化 | P14 | `Quant*` 平台类型、旧 finance 示例、金融计算 skill 脚本和旧评测语料按模块完成改名/重写；retail benchmark、type-check、vitest 回归通过 | open（本切片完成视觉验收公共契约的 `Quant*`→`Retail*` 改名及金融工作台文案零售化；剩余 generation-validation/mission control、scaffold 金融模板兼容层、eval 金融语料和 skill 脚本继续拆分） |
