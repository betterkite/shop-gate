...
| [ISSUE-P28](https://github.com/betterkite/shop-gate/issues/17) | 电商经营分析能力扩展 | P17 | 用户漏斗与留存、RFM、渠道/活动归因、毛利/利润、补货预测、商品生命周期和价格弹性形成可运行能力；每项能力有数据缺口提示、看板模板、后端聚合、E2E case 和桌面/移动验收 | in-progress（已完成 RFM、渠道/活动、毛利、库存健康、补货参考、商品经营阶段、用户留存、价格带与价格弹性边界 9 个后端接口；新增 `/analytics-workbench` BI 工作台、Agent 下钻接口/allowlist 和 `analytics-bi` 生成模板第一阶段；商品阶段支持全量分页、四阶段筛选和规则说明，用户留存支持首购周 cohort、完整 7 日观察窗口和 7 日留存率，补货参考支持全量分页、显式供货假设、优先级和原因说明，并接入生成式 BI 的预取、数据契约和看板模板；P28 R23 真实生成任务单例 E2E 已通过，`p28-experiment` 重跑同样 `ready=1/1`、校验通过且预览 HTTP 200；真实 DeepSeek 聊天单轮与第二轮追问均已确认 `answer_ready`、durable job `completed` 且无预览；本轮新增 `dataset_price_experiment_observations` 和合成对照/处理组模拟，后端、工作台和生成模板按 `estimated`/`synthetic_experiment_reference`/`insufficient_data_for_experiment` 状态展示观察关系、实验模拟参考或数据缺口；真实实验接入、随机分组证据、显著性检验和因果分析仍未完成，数据集导入联动另见 ISSUE-P31；证据见 [`docs/review/p28-analytics-capabilities.md`](docs/review/p28-analytics-capabilities.md)） |
...
| [ISSUE-P32](https://github.com/betterkite/shop-gate/issues/18) | 导入任务到 Agent 看板的跨服务持久化编排 | P17 | 数据集导入/更新后，在明确的 `project_id` 范围内消费 `analytics-bi` 配置清单；任务具备 outbox/幂等键、失败重试和证据链；Agent 生成并验证持久预览，任务中心可打开正确数据集且不覆盖其他项目或数据集 | in-progress（第一片已增加 `project_id`/`idempotency_key` 任务合同、项目范围查询、可重试事件 outbox、事件列表/ack 接口和工作台项目绑定入口；Agent 消费、持久看板生成/验证和可打开预览 URL 留待下一片；证据见 [`docs/review/p32-analytics-orchestration.md`](docs/review/p32-analytics-orchestration.md)） |
...
| [ISSUE-P36](https://github.com/betterkite/shop-gate/issues/19) | 公开真实 E2E benchmark runner 与隐藏集边界 | P0/P9 | `benchmark:commerce:e2e` 真实执行 `check-task-e2e` 并生成可审计报告；`--repeat`、模型和数据集可见性参数不被静默忽略；nightly/release-evidence 能启动完整前后端链路；hidden/shadow 集合只从受控外部输入读取，不混入公开仓库 | open（本次为保证公开仓库干净 checkout 可复现，已将 contract gate 收敛为公开数据集契约校验；真实 task-E2E 仍使用 `npm run check:task-e2e`，现有 benchmark 包装入口和 hidden/shadow 工作流需要下一片独立恢复，避免把运行证据检查误称为生成器） |
