...
| ISSUE-P5 | 最小闭环验收：自然语言零售问题 → 生成 dashboard 端到端 | P5 | 用户验收通过 | **待用户最终验收**——管线已全绿：DeepSeek rewrite ready → plan planned → prefetch 4 数据集 0 警告 → 确定性看板 → 验证 0F/0W → 证据验收通过 → 预览 http://localhost:4114；数据窗口=2026-08-29~09-06（时间平移演示口径，--time-shift last-week，页脚明示） |
