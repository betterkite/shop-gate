...
| ISSUE-P14 | generation/eval/scaffold 内部历史命名与模板语义零售化 | P14 | `Quant*` 平台类型、旧 finance 示例、金融计算 skill 脚本和旧评测语料按模块完成改名/重写；retail benchmark、type-check、vitest 回归通过 | open（本切片已完成 generation 状态/队列/终态内部契约改名；剩余 eval/scaffold/金融计算 skill 脚本继续拆分） |
