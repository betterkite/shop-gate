...
| ISSUE-P6 | 商品池/品类池/渠道分析全量重建（原 strategy-platform 6,299 LOC） | P6 | 1:1 映射完成，视觉 smoke 通过 | in-progress（P6a 已并 main `e854ffc`：**标准生成按 capability 分发到能力看板模板**，4 项能力模板修复——`meta` 声明、`.filter(Boolean)`→类型守卫、`svgBars/svgDailyLines` 用 `dangerouslySetInnerHTML` 真正渲染 SVG。P6b 已并 `421615f`：catalog 看板升级为**多面板**（GMV 柱状图 + Top-5 集中度环形图 + 分日趋势线），真实数据 `ready`、visual `svgCount=3`/`graphicCount=14`。P6c 已并 `684a435`：新增**商品池/渠道分析数据端点**（`/items` 支持分页/类目/排序，`/channels` 按店铺 tier 聚合），curl 验证 `/items` total=412,130、`/channels` 返回 standard/premium/flagship。余：`/commerce-platform` 商品运营前端页（商品池/品类池/渠道三视图）） |
