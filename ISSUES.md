...
| ISSUE-P5 | 最小闭环验收：自然语言零售问题 → 生成 dashboard 端到端 | P5 | 用户验收通过 | **待用户最终验收（真实数据口径）**——管线已全绿：DeepSeek rewrite ready → plan planned → prefetch 4 数据集 0 警告 → 确定性看板 → 验证 0F/0W → 证据验收通过 → 预览 http://localhost:4114；数据 = **真实天池 UserBehavior**（`behavior_source=tianchi_userbehavior`，10,000 用户 / 1,013,367 事件），窗口 2017-11-25~12-03（`--time-shift none` 保留原始日期）；金额/库存为合成主数据（GMV=buy×合成价），页脚明示 |
