...
| ISSUE-P7 | 经营情报中心全量（原 research-reports 891 LOC + 日报自动化） | P7 | 日报链路可运行 | in-progress（P7a 已并 main `e1ad56c`：零售**经营情报中心**——`retail-briefing.ts` 数据层（日报摘要/类目经营榜/观察池）+ `/operations-briefing` 零售三视图替换金融"投研情报中心"，登录态真实数据渲染 daily/categories=100/watch=20，合成徽标+来源标注。余：**日报自动化链路**——把日报生成并持久化为 OperationBrief/OperationBriefRun（参照金融 `runDailyResearchReport` 零售化），并接 `/api/commerce/briefing/daily` 路由） |
