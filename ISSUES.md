...
| ISSUE-P31 | 电商数据集选择与导入后分析联动 | P17 | BI 工作台提供数据集选择器，展示窗口、来源、行数和合成字段；切换数据集后所有分析视图同步读取目标 `dataset_id`；数据集导入完成后自动刷新可用数据集与当前分析，按配置异步触发看板生成；刷新/生成结果有状态、失败原因和证据记录，不重复生成、不覆盖用户正在查看的其他数据集 | in-progress（已完成选择器、自动刷新和受控合成数据集导入第一片：新增异步任务 API、`platform_jobs` 状态、质量扫描、失败原因和网页生成入口；真实 CSV/第三方数据导入、数据集到看板的异步编排、任务中心和商品级行为明细仍待后续片，验收记录见 [`docs/review/p31-dataset-linkage.md`](docs/review/p31-dataset-linkage.md)） |
