...
| [ISSUE-P28](https://github.com/betterkite/shop-gate/issues/17) | 电商经营分析能力扩展 | P17 | 用户漏斗与留存、RFM、渠道/活动归因、毛利/利润、补货预测、商品生命周期和价格弹性形成可运行能力；每项能力有数据缺口提示、看板模板、后端聚合、E2E case 和桌面/移动验收 | in-progress（已完成 RFM、渠道/活动、毛利、库存健康、补货参考、商品经营阶段、用户留存、价格带与价格弹性边界 9 个后端接口；新增 `/analytics-workbench` BI 工作台、Agent 下钻接口/allowlist 和 `analytics-bi` 生成模板第一阶段；商品阶段支持全量分页、四阶段筛选和规则说明，用户留存支持首购周 cohort、完整 7 日观察窗口和 7 日留存率，补货参考支持全量分页、显式供货假设、优先级和原因说明，并接入生成式 BI 的预取、数据契约和看板模板；P28 R23 真实生成任务单例 E2E 已通过，`p28-experiment` 重跑同样 `ready=1/1`、校验通过且预览 HTTP 200；真实 DeepSeek 聊天单轮与第二轮追问均已确认 `answer_ready`、durable job `completed` 且无预览；本轮新增 `dataset_price_experiment_observations`、用户级 outcomes 和合成对照/处理组模拟，后端、工作台和生成模板按 `estimated`/`synthetic_experiment_reference`/`aggregate_reference_only`/`ready_for_user_level_review` 状态展示观察关系、实验模拟或证据缺口；P28 后续 Issue #52 补充用户级统计复核和多实验未校正边界；正式因果模型、分层检验和多重检验校正仍未完成，数据集导入联动另见 ISSUE-P31；证据见 [`docs/review/p28-analytics-capabilities.md`](docs/review/p28-analytics-capabilities.md)） |
...
P28 后续 Issue #52：在完整真实 outcomes 上输出对照组/处理组的用户数、购买用户数、购买件数、购买率、差异区间和 p 值，统计字段只用于样本复核；多个实验明确标记 p 值尚未做多重检验校正，缺失或不完整 outcomes 保持数据缺口状态。正式因果模型、分层检验和校正方法仍留待后续切片。
