...
| ISSUE-P34 | 开源发布卫生与可复现性 | P0 | 活动 Skill、公开文档和产品元数据不再包含旧领域说明、个人绝对路径或旧仓库链接；仓库具备贡献、安全和行为规范；秘密、运行产物与本机路径扫描通过；Skill registry、lock、tgz 与源文件一致；全套发布前门禁通过 | done（公开仓库已创建：[`betterkite/shop-gate`](https://github.com/betterkite/shop-gate)；代码已上传到 `codex/shopgate-issue-p35-runtime-retailization` 分支并创建 [draft PR #1](https://github.com/betterkite/shop-gate/pull/1)，仓库可见性为 public；License 版权主体为 `betterkite`；本地 `release:check:full` 与第三轮 GitHub Actions 全部通过。合并 `main` 保留为用户审阅后的下一步） |
