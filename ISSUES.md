...
| ISSUE-P13 | commerce-data 后端旧金融模块物理清理 | P13 | 运行入口只保留零售数据路由；旧行情/回测/ClickHouse/provider/models/tests/config/docker 残留有引用图、迁移说明和后端全门验证 | open（本轮只确认范围，未执行批量删除；见交接审计记录） |
