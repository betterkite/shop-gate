...
| ISSUE-P6 | 商品池/品类池/渠道分析全量重建（原 strategy-platform 6,299 LOC） | P6 | 1:1 映射完成，视觉 smoke 通过 | in-progress（P6a 已并 main `e854ffc`：**标准生成按 capability 分发到能力看板模板**，4 项能力模板修复——`meta` 声明、`.filter(Boolean)`→类型守卫、`svgBars/svgDailyLines` 用 `dangerouslySetInnerHTML` 真正渲染 SVG；真实数据回归 `ready`、visual `svgCount=1`。余：商品池/品类池/渠道分析业务页与更富视觉面板） |
