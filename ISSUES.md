...
| ISSUE-P28 | 电商经营分析能力扩展 | P17 | 用户漏斗与留存、RFM、渠道/活动归因、毛利/利润、补货预测、商品生命周期和价格弹性形成可运行能力；每项能力有数据缺口提示、看板模板、后端聚合、E2E case 和桌面/移动验收 | in-progress（已完成 RFM、渠道/活动、毛利、库存健康、商品经营阶段、价格带边界 7 个后端接口；新增 `/analytics-workbench` BI 工作台、Agent 下钻接口/allowlist 和 `analytics-bi` 生成模板第一阶段（模板选择、扩展数据预取、renderer、前置校验）；商品阶段现支持全量分页、四阶段筛选和规则说明；仍需真实生成任务 E2E、真实聊天多轮 E2E、桌面/移动视觉验收，价格弹性继续明确数据不足；证据见 [`docs/review/p28-analytics-capabilities.md`](docs/review/p28-analytics-capabilities.md)） |
