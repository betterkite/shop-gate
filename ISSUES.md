...
| ISSUE-P31 | 电商数据集选择与导入后分析联动 | P17 | BI 工作台提供数据集选择器，展示窗口、来源、行数和合成字段；切换数据集后所有分析视图同步读取目标 `dataset_id`；数据集导入完成后自动刷新可用数据集与当前分析，按配置异步触发看板生成；刷新/生成结果有状态、失败原因和证据记录，不重复生成、不覆盖用户正在查看的其他数据集 | in-progress（已完成第一片：BI 工作台读取 `/api/v1/commerce/datasets`，提供契约驱动选择器，显示窗口/来源/契约版本/记录数/合成字段，切换后按 `dataset_id` 重新读取当前视图；导入后自动刷新、异步看板生成和商品级行为明细仍待后续片，验收记录见 [`docs/review/p31-dataset-linkage.md`](docs/review/p31-dataset-linkage.md)） |
