...
Shop Gate 是通用 Data Agent 平台上的第一个零售电商应用。核心链路是：用户提出经营问题，Data Agent 使用所选 LLM 形成通用任务合同，Retail Domain Pack 再通过独立 Resolver 核验商品/类目实体、生成零售 run plan 并注入数据、Skills、工具和 Mission；PI Agent 根据受控合同运行完整多轮工具循环，Shop Gate 治理层约束副作用并生成工作空间，最后由 Delivery 验证、视觉检查、产物契约和评测决定是否可交付。
...
flowchart LR
  ...
  G --> DB
  F --> S[Retail Skills / Tools / Validators]
  S --> R
  ...
  W --> T[商品运营 /commerce-platform]
  W --> C[业务知识中心 /business-knowledge]
...
| 数据线 | commerce 数据后端把零售数据源导入 PostgreSQL/TimescaleDB，页面和 Agent 都优先读本地库 |
| 生成线 | Agent Runtime 使用 skills 规划、取数、写页面，并把事件和产物写入 workspace |
| 质量线 | 验证、视觉检查、产物契约、评测和运维共同判断结果是否可交付 |

如果你在排查问题，可以先判断自己站在哪条线上。比如漏斗或商品指标缺数，多半是数据线；页面生成了但很丑，多半是生成线和 skills；预览打不开，多半是生成线、工作空间契约或基础环境；评测失败，多半要沿着质量线回看产物。

## 主链路

1. 用户输入问题，必要时上传截图。
2. 平台使用项目当前模型生成通用 `.data-agent/task.json` 与零售 `.data-agent/retail-query-rewrite.json`；模型负责语义，商品/类目实体由 `/api/v1/commerce/resolve` 独立确认。
3. 模型未配置、超时、失败或输出缺少原文字面证据时返回 `llm_unavailable` 并停止；不执行关键词降级。
4. `run-planner` 消费 Query Rewrite，信息不足时进入澄清；信息完整后同时生成 `.data-agent/plan.json` 和零售 `.data-agent/retail-run-plan.json`。
...
通用 Task/Profile/Domain Pack 合同、Domain Pack 解耦边界和新业务接入步骤见 [Data Agent 平台与 Domain Pack 架构](data-agent-architecture.md)。
...
Workspace 当前是“可持续演进的项目工作区”，不是每个 Job 的临时目录，也不是 Worker 的私有目录。因此同一 Workspace 不做隐式并行或静默串行排队；调用方必须等当前 Mission 完成、失败或取消后再提交下一次写入。未来如需同项目并行分析，应新增 branch/snapshot workspace 身份并在验证后显式合并，而不是放宽现有单写约束。
...
Shop Gate 当前采用 Python/Node 长期主线，不引入 Dubbo3 作为配置中心、服务发现或服务注册。对应能力由轻量服务目录承担：

- `config/service-catalog.json` 记录 web、commerce-data、TimescaleDB、Redis、Loki、Grafana 和 Alloy 的职责、runtime、endpoint、启动命令和依赖。
...
后端位于 `services/commerce-data`，当前以天池淘宝 UserBehavior 公开切片为行为数据源，商品/价格/库存等主数据为合成口径，通过 `shopgate-commerce-import` 导入 CLI 落库。响应统一携带数据来源标注；金额类字段必须带“合成口径”标注（`gmv = buy 事件数 × 合成价格`）。字段口径见 [数据字典](data-dictionary.md)。

后端代码按 Controller / Use Case / Repository / Core Helper 分层推进：

- `routers/` 只处理 HTTP 参数、状态码和响应模型。
- `services/` 编排 provider、缓存策略、降级和响应聚合。
- `repositories/` 承接 TimescaleDB/PostgreSQL SQL、分页、批量写入和读模型缓存。
- `database_core.py` 只保留连接、日期、Decimal、JSON 和实体元数据解析等无业务状态基础函数。
...
- TimescaleDB 承载 `commerce.user_behavior_events`、`commerce.daily_item_metrics`、`commerce.daily_category_metrics` 和 `commerce.*` 平台任务表等时序数据。
- Redis 承载短期缓存，优先用于渠道/指标摘要和后续任务进度。
- Loki/Grafana/Alloy 承载集中日志采集和运维排查；Loki 未启动时运行治理中心会降级读取本地文件日志。
- 零售数据源、导入 CLI 和 provider 书签见 [零售数据接入](commerce-data-ingestion.md)。
...
Shop Gate 当前最重要的取舍是“本地事实库优先”。外部切片可以字段有限，但只要数据已经进入本地 TimescaleDB，生成看板和评测都应该优先复用同一份事实。
...
| Redis 先做短期缓存 | 商品池摘要、渠道聚合和任务进度适合缓存，但长期结果仍写回数据库 |
| Loki 可选但推荐 | 本地开发可以降级到文件日志，排复杂问题时集中日志更省时间 |
| Skills 作为生成规则层 | 同类页面问题不应每次只修代码，要沉淀成下一次生成能复用的规则 |
| Python 作为 commerce 数据后端主语言 | 当前瓶颈主要是数据导入、IO、缓存、批处理和存储形态，不是 CPU 密集计算；优先强化 FastAPI + async/批处理 + Redis/TimescaleDB，而不是过早引入 Go 或 Rust |

这个架构允许组件分阶段增强：没有 Loki 时平台还能看本地日志；没有 Redis 时可以直读数据库；没有 commerce 数据后端时部分页面会降级展示注册表。但数据库和生成工作空间契约是核心，一旦缺失就很难保证结果可追溯。

## 后端语言边界

`services/commerce-data` 继续以 Python 为主线。短期不要为了“可能的性能问题”拆出 Go 或 Rust 服务，除非已经通过 profiling 证明瓶颈是 Python 运行时本身。

优先优化顺序：

1. 批量接口和批量写入，减少逐条串行 IO。
2. Redis 做短 TTL 热点缓存，TimescaleDB 做事实库。
3. 大扫描分析先依赖 TimescaleDB 聚合表；列式分析层暂缓引入，出现超大行为事件扫描需求时再评估。
4. 后台队列拆分长任务，避免阻塞请求链路。
5. 只有在 CPU 密集计算、极高并发网关或二进制协议服务成为明确瓶颈后，再考虑 Rust/Go。

Go/Rust 的合理引入场景：

- Rust：高频指标聚合、列式文件解析、极重 CPU 规则验证内核。
...
| 商品运营 | `/commerce-platform` | 商品池（分页/排序）、品类池（top-100）、渠道分层（3 tier） |
| 业务知识中心 | `/business-knowledge` | 查看业务能力、典型场景、交付契约和支撑依赖 |
| 经营情报 | `/operations-briefing` | 经营日报、类目经营榜、观察池和“生成今日日报”入口 |
