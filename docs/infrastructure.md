...
SHOPGATE_COMMERCE_API_REQUIRED=0
...
| `SHOPGATE_COMMERCE_API_ENABLED` | `1` | 是否探测 `services/commerce-data` 后端。关闭后业务知识中心的支撑资源视图展示内置注册表。 |
| `SHOPGATE_COMMERCE_API_REQUIRED` | `0` | 市场数据后端不可用时是否失败。 |
