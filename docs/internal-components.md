...
读它时可以想象一次真实请求：用户问一个经营问题，页面要创建项目，Agent 要规划，后端要取数，数据库要保存，skill 要生成页面，验证器要检查，运行治理中心要告诉我们哪里出错。下面这些组件就是这条路上的不同岗位。
...
| 零售领域层 | `src/lib/commerce/`、`src/lib/domains/retail/` | 零售能力、评测、经营情报、生成执行、工作空间健康、生成验证和 Skills 数据 |
| 运维领域层 | `src/lib/ops/` | 基础环境健康、日志、Docker/数据库/Loki 状态 |
| 数据库入口 | `src/lib/db/`、`prisma/` | Prisma 管理的主业务表 |
| commerce 数据服务 | `services/commerce-data/` | 零售指标、漏斗、商品/类目、导入任务和基础组件 API |
| SQL 初始化 | `sqls/` | `commerce` schema、TimescaleDB hypertable、零售事实表和平台任务表 |
...
这个链路里最重要的边界是：Agent 负责规划和生成，commerce 数据服务负责取数和入库，验证器负责判断结果是否可用。不要让一个组件承担所有责任。边界清楚，排障才不会乱。
...
| 商品运营 | `/commerce-platform` | 商品池分页、商品趋势详情、品类池、渠道分层 |
| 经营情报 | `/operations-briefing` | 经营日报、类目经营榜、观察池和“生成今日日报” |
| 业务知识中心 | `/business-knowledge` | 业务能力、典型场景、交付规范和支撑资源 |
| 运行治理中心 | `/ops-platform` | 运行总览、服务治理、工作空间交付、生成链路和日志现场 |
...
| 行为事件、商品/类目日聚合 | TimescaleDB `commerce` schema | 时序数据量大，需要按时间查询 |
| 日志、截图、大 JSON、生成源码 | 文件系统 / Loki | 原始产物大，数据库只保存索引和摘要 |

Redis 是短期缓存，不是事实库。缓存丢了不应该影响长期分析结果，只会影响速度或进度展示。

这条边界很重要。缓存可以让页面更快，但不能让分析结果只存在缓存里。凡是会影响经营看板、商品分析和指标解释的数据，都应该最终落到 PostgreSQL/TimescaleDB 或可追溯文件里。
...
| `database_core.py` | 数据库连接、日期、Decimal、JSON 和实体（商品/类目/品牌/店铺）元数据解析等基础函数 |
| `models.py` | Pydantic 响应模型和请求模型 |
| `routers/` | Controller 边界，新增 HTTP 入口优先落这里；零售读模型在 `routers/commerce.py` |
| `services/` | Use Case 边界，编排缓存、降级、provider 和 repository |
| `repositories/` | TimescaleDB/PostgreSQL 持久化边界，按领域拆分 repository（含导入任务书签、数据质量扫描和批量写入） |
| `import_cli.py` | `shopgate-commerce-import` 导入 CLI：UserBehavior 导入、合成数据生成和日聚合 |
| `cache.py` | 本地 JSON 缓存和 Redis 短期缓存 |

commerce 数据服务的原则是：外部切片只作为采集入口，本地 TimescaleDB 才是零售分析和看板生成优先读取的事实库。

这也是为什么我们会关心导入、字段口径和数据质量。切片今天能导入，不代表明天不需要重跑；本地库和 evidence 才能让一次分析可复盘。

旧 `database.py` 兼容门面已删除。后端能力的详细分层和迁移路线见 [后端能力架构与持续优化边界](backend-capability-architecture.md)。新增后端能力时先判断自己属于 Controller、Use Case、Repository、Provider Adapter 还是 Cache Aside，不要默认继续扩写 `api.py`。
...
| 数据 | `commerce-market-data`、`commerce-master-data` | 获取零售指标、商品主数据和行为数据 |
...
| 组件 | 位置 | 关注点 |
| --- | --- | --- |
| 自动验证 | `src/lib/commerce/retail-validation.ts` | build、HTTP、数据文件、证据和 stale report |
| 产物契约 | `src/lib/generation/artifact-contracts.ts` | 禁止远程资源、mock 数据和敏感信息 |
...
SHOPGATE_COMMERCE_API_ENABLED=1
...
新增组件时要同时考虑：它是否必须存在，缺失时怎么降级，doctor 和运行治理中心如何展示。
...
1. 在 `/commerce-platform` 打开一个商品，观察前端请求和后端 `GET /api/v1/commerce/items`（分页/类目/排序）、`GET /api/v1/commerce/items/{id}/daily` 的商品趋势数据。
