...
| 量化业务知识中心 | `/business-knowledge` | 业务能力、典型场景、交付规范和支撑资源 |
...
旧 `database.py` 兼容门面已删除。后端能力的详细分层和迁移路线见 [后端能力架构与持续优化边界](backend-capability-architecture.md)。新增后端能力时先判断自己属于 Controller、Use Case、Repository、Provider Adapter、Cache Aside 还是 Analytics Adapter，不要默认继续扩写 `api.py`。
