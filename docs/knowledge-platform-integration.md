...
Shop Gate 通过 AKEP v0.1 HTTP 协议使用独立的 Agent Knowledge Platform。这个接入用于已发布、可引用、受 Space 和 purpose 约束的共享知识，不替代零售数据事实库、用户记忆、Skills 或外部模型网关。

## 组件职责

```text
Agent Knowledge Platform -- AKEP HTTP --> Shop Gate KnowledgePort --> PI Agent
外部模型网关 -- OpenAI-compatible HTTP --> Shop Gate Provider Adapter --> PI Agent
commerce-data -- HTTP --> Shop Gate data prefetch --> workspace
```

- Agent Knowledge Platform 管理知识 Candidate、审核、发布、Revision、Citation、Exposure、Usage 和 Feedback。
- Shop Gate 管理用户/项目授权、RunPlan、Mission、Agent 上下文、工作空间、验证和最终交付。
- 外部模型网关管理 Qwen、DeepSeek 等模型的协议、路由、客户端鉴权、配额和用量。AKEP 不调用模型。
- commerce-data/TimescaleDB 仍是行为事件、商品主数据和零售指标事实的权威来源。
- Evolvable User Memory 只保存用户明确授权的个性化偏好。

两个仓库不共享数据库、Compose、源码包、文件路径或发布流程。Shop Gate 不使用 AKEP 私有 Console API，也不通过相对路径依赖 AKEP 仓库内尚未发布的 TypeScript SDK。

## 运行链路

1. Shop Gate 完成用户、项目和请求授权，并由受信 `run-planner` 生成 RunPlan。
...
4. Shop Gate 用配置固定的 Space、purpose、obligation 和字符预算请求 ContextPack。模型不能选择 AKEP URL、token、Space 或 purpose。
5. ContextPack 被包装成不可信 JSON capsule。系统提示明确禁止其中内容覆盖用户请求、真实业务事实、权限、Skills、工具合同、验证和风险控制。
6. 平台把 Citation、Revision、Payload digest、Policy Epoch 和 Exposure Receipt 写入 `evidence/knowledge-sources.json`，不建立正文镜像。
7. Shop Gate 在 Agent 调用前把 AKEP Exposure 与 Memory Usage 的不透明引用写入[联合上下文清单](context-composition.md)，不复制两边正文。
8. 只有 Mission 取得 accepted Evidence Receipt 后，Shop Gate 才按实际进入 Agent 上下文的 Citation 写 AKEP Usage；取消、拒绝、澄清和失败任务不写 Usage。
9. Shop Gate 将 Usage、Citation 绑定和 Mission receipt 写入服务端 `governed_knowledge_uses` 归因账本；工作空间 JSON 只用于审计展示，不能作为反馈授权依据。
...
- 反复成功的业务模式可由独立 contributor workload 结合 accepted Mission receipt 和脱敏业务指标生成 Candidate。Qwen/外部模型网关可以辅助形成候选草稿，但模型输出只标记为 `generatedBy`，不能自评、自审或自发布。
...
Agent Knowledge Platform 使用同尾号端口对：统一 Web 入口 `http://localhost:33005`、Core 直连 `http://localhost:38085`。Shop Gate/外部模型网关分别使用 `3000`/`38082`，端口职责互不重叠。
...
`SHOPGATE_KNOWLEDGE_SPACES` 是所有工作区都可读的共享 Space。每次请求还会由服务端追加 `<PROJECT_SPACE_BASE_URL>/<url-encoded Project.id>`；模型、浏览器 body 和生成 workspace 都不能选择或扩大它。生产 token 必须只授权所需 shared Space 与当前项目 Space。关闭 `PROJECT_SPACES_ENABLED` 表示明确采用 shared-only 模式，不适用于含项目私有知识的部署。
...
```dotenv
SHOPGATE_KNOWLEDGE_OAUTH_TOKEN_URL=https://identity.example/oauth2/token
SHOPGATE_KNOWLEDGE_OAUTH_CLIENT_ID=shopgate
SHOPGATE_KNOWLEDGE_OAUTH_CLIENT_SECRET=inject-from-secret-manager
SHOPGATE_KNOWLEDGE_OAUTH_RESOURCE=https://knowledge.example/akep/0.1
SHOPGATE_KNOWLEDGE_OAUTH_SCOPE="akep:query akep:read akep:feedback"
```

当前 AKEP Core 是固定单 Tenant 进程模型。生产 token 的签名 Tenant claim 必须与该部署完全一致；请求参数不能自报 Tenant。普通 Shop Gate workload 不获得 review、publish、incident 或 erase scope。

## 可用性语义

- `SHOPGATE_KNOWLEDGE_REQUIRED=0`：超时、401/403、契约不兼容或空结果显式降级，核心生成任务继续，不能伪造知识。
- `SHOPGATE_KNOWLEDGE_REQUIRED=1`：知识准备失败时任务在 Mission 创建前失败关闭。
- `offline` degradation mode：不访问 AKEP 和外部模型网关等外部可选依赖。
- 空 ContextPack 是合法结果；只说明当前授权 Space、purpose、词法检索和预算下没有匹配项。

Readiness 会分别展示 `knowledge` 与外部模型网关，不能用顶层 `ok=true` 代替组件检查。

## 外部模型网关与 Qwen

默认 profile `local_qwen:qwen3.5-9b-q5km` 继续通过外部模型网关 `/v1/chat/completions` 使用本地 Qwen。知识检索本身不消耗模型 Token；Qwen 只负责 Shop Gate 已有的 Query Rewrite、自定义生成和评测 lane。标准可信看板 lane 仍使用零模型 Token 的确定性工具计划。
...
- `src/lib/platform/knowledge/port.ts`：Shop Gate provider-neutral Port。
...
- `config/service-catalog.json`、`src/lib/ops/readiness.ts`：AKEP 与外部模型网关运维可见性。
