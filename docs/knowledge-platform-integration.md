...
Shop Gate 通过 AKEP v0.1 HTTP 协议使用独立的 Agent Knowledge Platform。这个接入用于已发布、可引用、受 Space 和 purpose 约束的共享知识，不替代行情事实库、用户记忆、Skills 或 ModelPort。
...
commerce-data -- Quant HTTP --> Shop Gate data prefetch --> workspace
```
...
- commerce-data/TimescaleDB 仍是行情、财务、因子和回测事实的权威来源。
...
- `SHOPGATE_KNOWLEDGE_REQUIRED=0`：超时、401/403、契约不兼容或空结果显式降级，核心量化任务继续，不能伪造知识。
