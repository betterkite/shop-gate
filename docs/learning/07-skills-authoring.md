...
目标：学会阅读、修改、发布和验证 Shop Gate Skills，并理解 skill 如何影响生成页面质量。
...
Skill 可以理解成 Agent 的本地专业手册。模型本身有通用能力，但它不知道 Shop Gate 当前有哪些数据文件、验证规则、页面模板、禁止事项和数据源边界。Skill 把这些项目规则写下来，让 Agent 在生成时遵守。
...
| `agents/openai.yaml` | UI 展示名、短描述和显式包含 `$skill-id` 的默认调用示例 |
| `references/` | 必需；字段合同、模板矩阵、案例和失败边界，必须由 `SKILL.md` 直接链接 |
| `scripts/` | 必需；至少一个可执行的确定性校验或计算脚本，支持 `--help`、稳定 JSON 输出和非零失败码 |
| `assets/` | 模板资源或示例素材，尽量少用，避免包过大 |

`SKILL.md` 应该短而硬。复杂解释放到 references，稳定计算放到 scripts，不要把所有东西都堆在入口文件里。只有一份 `SKILL.md` 的目录不是可发布 Skill；reference 或 script 没有从入口直接导航也视为不完整。
...
npm run package:skills -- dashboard-visualization
npm run check:skills
```
