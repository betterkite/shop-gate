...
> **说明：本文档描述运行治理中心（运行/巡检/评分）的通用机制；当前业务数据、任务健康和数据源指标均以 `commerce.*` schema 与零售能力为准。**

---

# 运行治理中心使用与评分指南

运行治理中心负责回答一个朴素但关键的问题：这个系统现在到底靠不靠谱？
...
| 运行日志 | Loki 集中日志、本地 Next 日志、评测队列日志和运行日志 |

运行总览还会并列展示项目健康、运行健康与经营数据健康三层画像。这样历史工作空间失败、可选日志组件降级和数据更新时间异常不会被混成一个笼统的“平台不可用”。
...
运行治理中心的评分是用于定位风险的工程评分，不是业务价值评分。它的原则是：阻断项扣得重，warning 扣得轻，缺少可选组件在 `auto` 模式下不直接让系统不可用。
...
运行治理中心还会给每个工作空间增加交付分层，用来避免历史失败项目污染主平台可用性判断：

| 分层 | 含义 |
| --- | --- |
| `可演示` | 关键产物和验证状态可用于展示 |
| `有风险` | 可以追踪，但存在 warning、未知验证或数据质量风险 |
| `待修复` | 存在验证、产物契约、视觉验收阻断，或仍有队列任务 |
| `归档候选` | 长时间未活跃且存在阻断项，应从主可用性口径中分离 |
