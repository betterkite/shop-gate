...
- 可终止、可观测：每个物理执行使用新的服务端 UUID `runInstanceId`，并派生带 `pi_agent_` 前缀的 runtime `runId`；运行具有结构化公共事件、累计 usage、轮数/Token/总时长预算和 AbortSignal。
...
| `model_custom` | 项目模型 Provider | 8 / 12 | 24,000 | 144,000 | 24,000 |
...
PI Agent 当前实现的是 **replan recovery 基础**，不是原会话 resume。DeepSeek thinking tool turn 所需的私有 reasoning 不写数据库，因此进程重启后不能完整重放旧协议历史。Checkpoint 固定声明 `recoveryMode=replan_required`；v2 包含阶段、turn、源事件序号、已终结 operation ID，以及只由框架哈希组成的 ProgressOracle 快照。v2 state hash 使用 canonical JSON 计算，避免 PostgreSQL jsonb 键顺序变化造成误判；恢复审计在取得 reconciliation lease 或执行回滚前完成 hash/schema 校验。旧 v1 checkpoint 仍按 replan-only 兼容读取。
...
- 新 Provider 实现 `PiAgentModelProvider`，不得让其 wire format 泄漏到 Run Engine。
- 新工具实现 `PiAgentTool`，必须声明 JSON Schema、输入校验、AbortSignal、超时、有界输出、`effect` 和 `idempotency`；漏声明 effect 会被保守当成 `external_write/reconcile_required`，不要依赖默认值获得更宽执行语义。需要人工控制的 mutating tool 还必须由受信应用声明 `approval` 与无凭据的完整公开投影；含隐藏参数的工具不得开放 edit。只有框架内建工具可注册逐工具 `projectContextReceipt`，projector 必须浅层校验固定字段并只返回 bounded canonical target/hash/bytes；产品 additional tool 的 receipt/approval projector 都会被剥离。只有结果完全由当前 workspace generation 决定的只读工具才能声明 `observationCache: 'workspace_generation'`。
- 新 AgentRun 终态只能通过结构化 `PiAgentRunResult` 和 `run_finished` 事件表达；产品完成态只能由当前 candidate version 的 accepted evidence receipt 驱动。
- 内部 `PiAgentEvent` 的工具事件可包含 raw arguments/result，只允许受信的进程内消费者读取；数据库、Message、SSE/WebSocket 和其他产品边界必须分别使用显式安全投影，不得直接序列化内部事件或完整 `PiAgentRunResult`，也不得保存内部 messages、hidden reasoning、raw tool arguments/result 或 raw cause。
- Provider 必须拒绝响应与工具调用身份缺失、身份漂移、重复 ID 和不完整终止，不能容错拼接成可执行动作。
- 新 Skill 必须进入 registry/lock，并在 `config/pi-agent-skill-capsules.json` 声明 phase、typed-tool 依赖、原子步骤、完成条件和可选 reference selector。发布检查会拒绝缺失 capsule、越界引用或包含 Bash/MCP/curl/npm 执行指令的 runtime capsule。
...
```bash
PI_AGENT_TEST_DATABASE_URL='postgresql://...' npm run test:pi-agent:postgres
```
...
每条新 AgentRun 持久化 `frameworkVersion=pi-agent:0.82.1` 与 `buildRevision`。CI/deployment 优先使用显式 revision；本地 dirty checkout 会把 tracked diff 以及所有 untracked path、类型、长度和内容 SHA-256 纳入有界 fingerprint，无法完整读取时显式标记 `dirty.unavailable`，不会冒充干净 HEAD。历史运行身份保持不可变，不会被重写成当前 PI 版本证据。

确定性 contract benchmark 只验证平台合同，不能证明模型 Agent 能力。正式发布证据把三类结果分开验收：默认本地 Qwen live-model cases、零模型 `deterministic_standard` product control，以及 repair/cancellation/crash runtime controls。live-model 证据必须由真实 `/act -> Mission -> EvidenceVerifier -> accepted receipt` 链路产生，并同时匹配当前 framework/build/git、OpenAI-compatible provider/Qwen model、request identity、完整 case 集、时间顺序和 receipt hash；`unversioned:*` 与 `dirty.unavailable` 构建不能作为发布证据。每个 case 保存逐物理 run 的有界安全摘要，accepted receipt 必须绑定 lineage 中状态为 `candidate_complete` 的 source run，候选来源必须是 `pi_agent_submit_result`，且 ledger 至少证明一次成功 workspace write 与一次 `submit_result`；平台恢复或安全模板候选不能冒充 PI Agent 能力成绩。live-model Token 证明按 run 与聚合双重要求 `input > 0`、`cached + cacheMiss = input`、`input + output = total`；deterministic product control 则要求全部 usage 分量严格为零并证明 `apply_dashboard_spec -> submit_result`。字段缺失或把 cache miss 伪装为零都不能通过效率门。
...
| `PI_AGENT_PROVIDER_RETRY_MAX_MS` | `10000` | 单次 Provider 重试等待上限，包括服务端 `Retry-After` |
| `PI_AGENT_TOOL_OUTPUT_CHARS` | `6000` | 单个 typed tool 的模型可见输出上限；结构化工具会在该预算内继续投影或返回分批查询提示 |
| `PI_AGENT_MAX_TURNS` | 未设置 | 可选的更严格上限，不能突破 PhaseGraph：standard 为 2 个确定性工具回合，custom 为 8 个模型回合，repair 为 3 个模型回合，data-preparation 为 8 个模型回合 |
| `PI_AGENT_MAX_TOTAL_TOOL_CALLS` | 未设置 | 可选的更严格上限，不能突破 PhaseGraph 的 standard 2、custom 12、repair 8、data-preparation 20 次累计工具调用预算 |
| `PI_AGENT_PRE_WRITE_READ_ONLY_TURNS` | `3` | 首次成功写入前允许的连续只读轮数；达到后 Provider schema 保持不变，执行器从下一轮拒绝 read 调用 |
| `PI_AGENT_POST_WRITE_READ_ONLY_TURNS` | `2` | 成功写入后允许的连续只读轮数；达到后 Provider schema 保持不变，执行器再次硬切回写入/提交阶段 |
| `PI_AGENT_MAX_TURN_OUTPUT_TOKENS` | `12000` | 单次 Provider 调用的输出 Token 上限 |
| `PI_AGENT_MAX_RUN_OUTPUT_TOKENS` | 未设置 | 可选的更严格上限，不能突破 PhaseGraph 的 standard 1（实际 usage 为 0）、custom 8,000、repair 6,000、data-preparation 16,000 |
| `PI_AGENT_CONTEXT_WINDOW_TOKENS` | `128000` | PI Agent 内部上下文治理窗口；是产品保守值，不等同于 Provider 宣传上限 |
| `PI_AGENT_MAX_INPUT_TOKENS` | `48000` | 每次 Provider 请求的内部输入预算；运行时还会收紧到 PhaseGraph 的 custom 24k、repair 20k、data-preparation 60k 单请求上限，并按本 run 剩余累计 prepared-input 额度逐轮继续收紧。data lane 因此默认实际单请求上限仍为 48k。估算包含最终 request-local envelope，采用带安全余量的多语种启发式并对高熵长串提高权重 |
| `PI_AGENT_CONTEXT_CAPSULE_MAX_BYTES` | `2048` | 单次 Trusted ContextCapsule 上限；显式配置只能进一步收紧。超限先按优先级淘汰，仍放不下则保留 canonical 历史并跳过压缩，不让压缩失败影响工具正确性 |
| `PI_AGENT_MAX_RUN_INPUT_TOKENS` | `160000` | 累计输入 Token 上限；同时收紧网络前的累计 full prepared 预留，但不改变单请求或 cache-miss 上限。默认有效累计 prepared 为 custom 144k、repair 60k、data-preparation 160k。优先使用严格校验的 Provider usage，缺失时按完整 prepared request 保守估算 |
| `PI_AGENT_MAX_RUN_CACHE_MISS_INPUT_TOKENS` | 未设置 | 独立的 Provider 回报后累计 cache-miss breaker；可选值只能更严格，不能突破 PhaseGraph 的 standard 1（实际 usage 为 0）、custom 24,000、repair 20,000、data-preparation 60,000。它不收紧 full prepared 预留；Provider 缺少 cache breakdown/usage 时才把完整输入保守计为 cache miss |
| `PI_AGENT_TIMEOUT_MS` | `1200000` | 总运行超时 |
| `PI_AGENT_LEASE_TTL_MS` | `60000` | Durable run lease 有效期 |
| `PI_AGENT_HEARTBEAT_INTERVAL_MS` | `15000` | Durable run 独立心跳间隔，必须小于 lease TTL |
| `PI_AGENT_APPROVAL_POLL_INTERVAL_MS` | `500` | 活跃 Worker 查询 pending tool approval 的间隔，允许 1–10000ms；只影响已显式声明 approval 的 mutating tool，不会全局开启审批 |
| `PI_AGENT_MISSION_VERIFICATION_LEASE_TTL_MS` | `120000` | Mission validation/evidence 独占租约；进程崩溃后到期候选才允许被新 worker 接管 |
| `PI_AGENT_MISSION_VERIFICATION_HEARTBEAT_INTERVAL_MS` | `30000` | Mission 验证独立心跳，必须小于 verification lease TTL；心跳失败后当前 worker 失败关闭 |
| `PI_AGENT_GENERATION_LEASE_TTL_MS` | `120000` | 项目级 planning/data-prefetch、Agent execution、manual validation 外层编排租约；数据库时钟到期后才允许接管 |
| `PI_AGENT_GENERATION_HEARTBEAT_INTERVAL_MS` | `30000` | 外层编排独立心跳，必须小于 generation lease TTL；失去 fencing 后当前 orchestrator 失败关闭 |
| `PI_AGENT_DISPATCH_LEASE_TTL_MS` | `120000` | durable generation job worker claim 有效期；只使用 PostgreSQL 权威时钟 |
| `PI_AGENT_DISPATCH_HEARTBEAT_INTERVAL_MS` | `30000` | dispatch worker 独立心跳，必须小于 dispatch TTL；取消/接管后旧 worker 被 fencing |
| `PI_AGENT_DISPATCH_PENDING_ORPHAN_GRACE_MS` | `120000` | job 已入库但未 claim 的崩溃窗口宽限期；到期且无细粒度活 lease 时封存为 replan-required |
