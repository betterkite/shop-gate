...
2. Agent 下钻接口已可用；多轮计划继承的确定性回归已通过（上一轮全库口径 → 本轮库存追问会得到 `price_inventory`、`broadUniverse=true`、`outputIntent=answer` 且不再澄清），但真实 DeepSeek 第二轮仍需在 durable queue 收尾修复后复跑，验证“用户追问 → Agent 选择接口 → 保留上轮上下文 → 输出可理解结论”；
...
- 真实多轮回归仍待复跑：应验证第二轮不错误要求重新指定全库对象、终态仍为 `answer_ready`，并确认同一请求不会出现预览地址或看板验收事件。
