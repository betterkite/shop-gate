...
1. `analytics-bi` 生成模板的单例真实生成任务 E2E 已通过；真实 DeepSeek 聊天入口的“只做问答”单轮验收已通过：可见中文回答、终态 `answer_ready`、`previewUrl=null`，没有看板验证/预览事件；仍需要扩大到真实多轮任务，验证 Agent 会按用户追问选择正确分析能力，而不是只依赖固定 capability；
2. Agent 下钻接口已可用；多轮计划继承的确定性回归已通过（上一轮全库口径 → 本轮库存追问会得到 `price_inventory`、`broadUniverse=true`、`outputIntent=answer` 且不再澄清）。真实复跑曾被首页默认能力覆盖和 Agent Runtime 5 秒事务超时阻断，现已分别修复为问答语义优先和 30 秒事务收尾，仍需重新完成真实第二轮，验证“用户追问 → Agent 选择接口 → 保留上轮上下文 → 输出可理解结论”；
...
- 真实多轮回归仍待复跑：应验证第二轮不错误要求重新指定全库对象、库存问题不被首页默认漏斗能力覆盖、终态仍为 `answer_ready`，并确认同一请求不会出现预览地址或看板验收事件。
