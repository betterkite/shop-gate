...
这篇文档说明如何把独立项目 `evolvable-user-memory` 接入 Shop Gate，以及接入后怎样写入偏好、验证聊天是否使用了记忆、提交可归因反馈和排查常见故障。
...
Shop Gate 当前把 Memory 作为可选外部服务使用。聊天执行、项目 API 和账号级使用控制已经接通；用户可在 `/account/memory` 明确启用或暂停未来任务的个性化。

| 能力 | 当前入口 | 行为 |
| --- | --- | --- |
| 用户使用控制 | Shop Gate `/account/memory` | 未明确启用时不向 Memory 发起任务召回；变更写入本地审计 |
| 显式新增偏好 | 项目对话输入区“记住偏好”/ Shop Gate 项目级 Memory API | 用户确认后创建证据、候选和第一条不可变 revision；不自动保存聊天全文 |
| 候选偏好确认 | 用户消息下方候选卡 | 只在高置信稳定表达时提示；交易、授权和一次性要求被过滤；不自动写入 |
| 查看与纠正偏好 | `/account/memory` 或 Shop Gate Memory API | 查看当前值、追加修订、保留历史版本 |
| 聊天自动召回 | `/api/chat/:projectId/act` | 每轮执行前召回，并把通过过滤的有界 capsule 交给 PI Agent |
| 使用归因 | Memory `/v1/usages` + Shop Gate PostgreSQL `external_memory_uses` | Memory 重建投影并签发 Usage Receipt；Shop Gate 只保存 usage/trace/revision 不透明 ID、策略版本和内容哈希 |
| 结果反馈 | 已验证完成消息下方“有帮助 / 不适合我” | UI 先读取 Shop Gate 本地归因；持久收据保证重试幂等并拒绝矛盾反馈 |
| 价值透明度 | `/account/memory`“实际价值闭环” | 只统计真正交给 Agent 的 revision 和明确反馈；旧版空归因单独隔离 |
| 可视化管理 | Memory 自带工作台 `http://127.0.0.1:33009` | 用于理解和调试 Memory 自身，不代替 Shop Gate 项目授权 |

管理 Shop Gate 用户偏好时应调用 Shop Gate API，不要让浏览器绕过 Shop Gate 直接调用 Memory。这样项目权限、可信用户身份、键白名单和归因账本才不会被跳过。
...
关闭后 Shop Gate 不发起 Memory discovery、recall 或 outcome 请求，不需要 `SHOPGATE_MEMORY_API_URL`、Bearer Token 或 Token Broker。聊天个性化状态为 `disabled`，账号记忆管理、偏好召回和本轮偏好反馈不可用；模型、LLM Query Rewrite、市场数据、工作空间生成、自动验证和预览继续正常工作。
...
下一轮同一用户发起研究任务时，Shop Gate 会用当前问题、产品、项目和能力上下文召回记忆。匹配成功并完成二次过滤后，聊天消息 metadata 会先记录“待使用”：
...
| 候选偏好真正交给 Agent | Shop Gate 写入本轮 revision 归因；最终回复显示反馈入口 |
...
| 项目级偏好用于其他项目 | 被 Shop Gate 二次过滤，不进入 Agent |
...
sequenceDiagram
  ...
  participant Q as Shop Gate API
  participant M as Memory API
  participant P as Shop Gate PostgreSQL
...
Shop Gate 不根据应用版本号猜测能力，而是读取服务根路径的 `api_contract` 和 `capabilities`：
...
SHOPGATE_MEMORY_TOKEN_BROKER_URL=
SHOPGATE_MEMORY_TOKEN_BROKER_CLIENT_ID=
SHOPGATE_MEMORY_TOKEN_BROKER_CLIENT_SECRET=
SHOPGATE_MEMORY_TOKEN_AUDIENCE=evolvable-memory-api
```

| 变量 | 建议与含义 |
| --- | --- |
| `SHOPGATE_MEMORY_ENABLED` | `1` 启用；`offline` 降级模式会覆盖并禁用外部集成 |
| `SHOPGATE_MEMORY_REQUIRED` | 本地推荐 `0`；召回失败时继续核心任务。只有业务明确要求 fail-closed 时才设为 `1` |
| `SHOPGATE_MEMORY_REQUIRE_PRODUCTION_READY` | 生产默认开启；拒绝连接仍有隐私、身份或审计阻塞项的实例 |
| `SHOPGATE_MEMORY_API_URL` | HTTP(S) 服务根地址，不允许在 URL 中嵌入凭据 |
| `SHOPGATE_MEMORY_TENANT_ID` | Shop Gate 这一消费应用独占的稳定 tenant，最长 128 字符；其他产品不得复用 |
| `SHOPGATE_MEMORY_TIMEOUT_MS` | 单次外部请求超时，范围 100–30000 ms |
| `SHOPGATE_MEMORY_RECALL_LIMIT` | 单轮最多召回条数，范围 1–100 |
| `SHOPGATE_MEMORY_MAX_CONTEXT_CHARACTERS` | 进入 Agent 前的 projection 字符预算 |
| `SHOPGATE_MEMORY_BEARER_TOKEN` | 只用于本地或单 subject 调试；多用户生产静态 token 会被配置校验拒绝 |
| `SHOPGATE_MEMORY_TOKEN_BROKER_*` | 生产身份服务入口和客户端凭据；按 tenant/subject/purpose 换取 60–3600 秒短期 token |
| `SHOPGATE_MEMORY_PRODUCTION_PROBE_SUBJECT_ID` | 非人类生产探针 subject；必须有有效 `personalization` ProcessingGrant，仅用于真实授权读路径门禁 |
| `SHOPGATE_MEMORY_TOKEN_AUDIENCE` | Memory JWT audience，必须与 Memory 的 `EMF_AUTH_JWT_AUDIENCE` 一致 |

部署 Shop Gate 本地归因表，然后重启 Web 进程使环境变量生效：
...
不要让两个项目共用 PostgreSQL。`ExternalMemoryUse` 不是 Memory 数据副本，只是 Shop Gate 对“这一轮到底使用了哪些 revision”的本地审计账本。
...
- Shop Gate 日志中没有 `INCOMPATIBLE_CONTRACT`、`NETWORK_ERROR` 或 `TIMEOUT`。

Memory 是 optional 时，`/api/ready` 总体仍可能返回 `ok=true`，所以不能只看顶层状态，必须同时看 `components[].name=memory`。

## 第四步：用户明确启用

登录 Shop Gate 后打开 `/account/memory`，点击“启用个性化”。未配置记录默认关闭；聊天会显示 `opted_out`，并且不会向 Memory 发送 query。暂停时在远端 projection 返回后、进入 Agent 前还会再次检查，降低并发撤回期间继续暴露的风险。

当用户说“以后回答时先给结论，再列证据和风险”这类稳定偏好时，Shop Gate 会在用户消息下显示候选卡。候选识别完全发生在 Shop Gate 本地，只进入消息 metadata；只有用户检查类型、内容和范围并点击“确认并保存”后，才调用版本化 HTTP 契约。关闭候选卡或忽略不会给 Memory 创建负反馈。

关闭只阻止 Shop Gate 在未来任务中使用外部记忆，不等于删除远端 Evidence、Revision、Trace 或 Outcome。Memory 删除编排上线前，界面和 API 都不得把关闭描述成删除。

## 第五步：写入并使用第一条偏好

项目 Memory API 使用 Shop Gate 自己的项目授权。认证关闭的本地开发环境可以直接使用 `curl`；认证开启时，应先正常登录，再在浏览器控制台运行下面的同源 `fetch` 示例，浏览器会自动携带会话 Cookie。不要为了调试绕过项目权限。

### 1. 显式新增偏好

在已登录的 Shop Gate 页面控制台中执行，并替换真实项目 ID：
...
该列表只返回 `context.product=shopgate` 且键前缀通过白名单的偏好。读取不会增加信念、支持度或效用。
...
已验证完成的 Shop Gate 回复会自动查询本地归因。存在实际暴露的 revision 时，回复下方显示“有帮助 / 不适合我”；点击后分别提交 `helpful` 或 `rejected` Outcome。只有用户明确接受、拒绝、纠正或评价本轮结果后，才提交 Outcome：
...
允许的结果类型是 `helpful`、`accepted`、`harmful`、`rejected` 和 `corrected`。Shop Gate 会先查本地归因账本；不属于本轮的 revision 返回 `MEMORY_REVISION_NOT_EXPOSED`，防止把无关结果记到某条偏好上。
...
- 生产环境应启用 Memory JWT identity，并由可信 token broker 为每个 tenant/subject/purpose 签发短期 token。Shop Gate 按 Scope 缓存，不会跨用户复用。
...
写模式只使用 `shopgate-long-term-integration-check-v1` 合成 subject 和 `output.answer_style` 合成项目偏好，不读写真实用户偏好。它会留下可审计的合成 preference、recall、归因和 Outcome；因此只在发布验收、契约升级或故障恢复后显式运行，不放进高频探活。
...
- Qwen 能在 Shop Gate 构造的有界 personalization prompt 上完成强制工具调用和工具结果续写。
...
| 新增/列表 API 返回 502 | 网络、超时、无效 JSON或外部契约错误 | 查 Shop Gate 日志中的安全错误码和 `x-request-id`，再查 Memory 日志 |
| 聊天为 `empty` | 没有匹配偏好，或 product/project/key 被二次过滤 | 检查 `context.product=shopgate`、项目 scope 和允许的键前缀 |
...
| 认证启用后 API 返回 401/403/404 | 未登录、缺少 capability 或没有项目角色 | 按正常 Shop Gate 登录和项目授权链路处理，不要直连 Memory 绕过 |
...
| `src/lib/platform/memory/port.ts` | Shop Gate 使用的 provider-neutral 端口 |
...
| `src/lib/platform/memory/control.ts` | Shop Gate 本地用户使用围栏 |
...
| `src/lib/platform/memory/repository.ts` | Shop Gate 本地使用归因账本 |
