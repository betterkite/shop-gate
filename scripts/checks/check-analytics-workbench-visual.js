...
async function inspectProfile(browser, storageState, profile) {
  ...
  try {
    ...
    await page.getByRole('heading', { name: '当前下钻范围的日趋势' }).waitFor({ state: 'visible', timeout: 20_000 });
    const state = await page.evaluate(() => {
      ...
      return {
        ...
        trend: text.includes('当前下钻范围的日趋势'),
        scope: text.includes('下钻条件：') && text.includes('1000009'),
        metrics: text.includes('页面浏览量（PV）') && text.includes('购买转化率'),
        table: document.querySelectorAll('table').length > 0,
        keepFilterLink: [...document.querySelectorAll('a')].some((link) => link.textContent?.includes('在总览中保留此筛选')),
        contextLink: [...document.querySelectorAll('a')].some((link) => link.textContent?.includes('打开当前下钻视图')),
      };
    });
