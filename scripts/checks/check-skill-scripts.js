...
const pycacheDir = fs.mkdtempSync(path.join(os.tmpdir(), 'shopgate-skill-pyc-'));
...
if (pythonScripts.length > 0) {
  const compile = spawnSync('python3', ['-m', 'py_compile', ...pythonScripts], {
    cwd: root,
    env,
    encoding: 'utf8',
  });
...
for (const script of scripts) {
  ...
  const command = script.endsWith('.py') ? 'python3' : script.endsWith('.sh') ? 'bash' : 'node';
