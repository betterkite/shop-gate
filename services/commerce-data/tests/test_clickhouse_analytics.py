...
from shopgate_commerce_data import clickhouse
from shopgate_commerce_data.api import create_app
from shopgate_commerce_data.clickhouse import get_clickhouse_health, is_clickhouse_enabled


@pytest.mark.anyio
async def test_clickhouse_health_is_disabled_by_default(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("SHOPGATE_CLICKHOUSE_ENABLED", raising=False)
...
def test_registry_exposes_clickhouse_analytics_provider() -> None:
    ...
    assert "shopgate-clickhouse-analytics" in provider_ids
