...
function addProjectQuery(query: URLSearchParams, projectId?: string): void {
...
function hrefFor(
  ...
  addProjectQuery(query, projectId);
  return `/analytics-workbench?${query.toString()}`;
}

function lifecycleHref(datasetId: string, page: number, stage = '', dimension?: string, value?: string, projectId?: string): string {
...
function customerHref(datasetId: string, page: number, dimension?: string, value?: string, segment?: string, projectId?: string): string {
  const query = new URLSearchParams({
    view: 'customers',
    dataset_id: datasetId,
    page: String(page),
...
function filteredOverviewHref(datasetId: string, dimension: string, value: unknown, projectId?: string): string {
  return hrefFor('overview', datasetId, dimension, String(value), projectId);
}
...
function ScopeNotice({ dimension, value, datasetId, projectId }: { dimension: string; value: string; datasetId: string; projectId?: string }) {
  ...
  return <div className="rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 text-xs leading-5 text-sky-950">
    <div className="flex flex-wrap items-center justify-between gap-2">
      ...
      <Link href={hrefFor('overview', datasetId, undefined, undefined, projectId)} className="shrink-0 font-semibold text-primary hover:underline">清除筛选</Link>
    </div>
...
function trendDateHref(payload: JsonRecord | null, dateValue: string, projectId?: string): string | null {
...
function TrendChart({ payload, title, description, projectId }: { payload: JsonRecord | null; title: string; description: string; projectId?: string }) {
  ...
  return <Panel title={title} description={description}>
    {rows.length === 0 ? <p className="text-sm text-muted-foreground">当前范围没有可展示的日趋势数据。</p> : <>
      ...
      <div className="grid gap-2">{rows.map((row) => {
        ...
        const dateHref = trendDateHref(payload, dateValue, projectId);
...
function ElasticitySummary({ payload, datasetId, filterDimension, filterValue, priceBand, projectId }: { payload: JsonRecord | null; datasetId: string; filterDimension?: string; filterValue?: string; priceBand?: string; projectId?: string }) {
  ...
  return <>
    ...
    <div className={`mt-5 rounded-xl border p-4 text-sm leading-6 ${experimentReady ? 'border-sky-200 bg-sky-50 text-sky-950' : 'border-slate-200 bg-slate-50 text-slate-800'}`}>
      ...
      {experimentReady && experimentRows.length > 0 ? <DataTable><thead className="bg-white/70"><tr className="text-left text-xs font-semibold text-muted-foreground"><th className="px-3 py-3">实验</th><th className="px-3 py-3">对照价格</th><th className="px-3 py-3">处理价格</th><th className="px-3 py-3">对照购买率</th><th className="px-3 py-3">处理购买率</th><th className="px-3 py-3">购买率变化</th><th className="px-3 py-3">变化范围（95%）</th><th className="px-3 py-3">样本判断</th><th className="px-3 py-3">分组证据</th></tr></thead><tbody className="divide-y divide-border/60">{experimentRows.map((row) => { const status = text(row.significance_status); const label = status === 'significant' ? '样本显示有差异' : status === 'small_sample' ? '样本较少，暂不判断' : '暂未发现明确差异'; const evidence = asRecord(row.assignment_evidence); const evidenceStatus = text(evidence?.status); const evidenceLabel = evidenceStatus === 'declared_randomized' ? (evidence?.synthetic ? '已记录分组（演示）' : '已记录分组') : evidenceStatus === 'not_verified' ? '未确认随机分组' : evidenceStatus === 'incomplete' ? '分组记录不完整' : '未提供用户分组'; return <tr key={String(row.experiment_id)}><td className="px-3 py-3 text-sm">{text(row.experiment_id)}</td><td className="px-3 py-3 text-sm">{displayMoney(row.control_price)}</td><td className="px-3 py-3 text-sm">{displayMoney(row.treatment_price)}</td><td className="px-3 py-3 text-sm">{displayPercent(row.control_conversion_rate)}</td><td className="px-3 py-3 text-sm">{displayPercent(row.treatment_conversion_rate)}</td><td className="px-3 py-3 text-sm font-semibold">{displayPercent(row.absolute_conversion_lift)}</td><td className="px-3 py-3 text-sm">{displayPercent(row.absolute_conversion_lift_ci_low)} ～ {displayPercent(row.absolute_conversion_lift_ci_high)}</td><td className="px-3 py-3 text-sm"><span>{label}</span><br /><span className="text-xs text-muted-foreground">p 值 {row.p_value == null ? '—' : text(row.p_value)}</span></td><td className="px-3 py-3 text-sm"><span>{evidenceLabel}</span><br /><span className="text-xs text-muted-foreground">对照 {displayNumber(evidence?.control_assigned_users)} 人 / 处理 {displayNumber(evidence?.treatment_assigned_users)} 人</span></td></tr>; })}</tbody></DataTable> : <p className="mt-2 text-xs">需要同时具备对照组、处理组、每组曝光人数、每组购买人数和分组方式，才会展示实验对比。</p>}
      <p className="mt-2 text-xs text-muted-foreground">变化范围表示按当前样本估算的 95% 区间；“样本显示有差异”不等于已经证明价格导致变化。合成实验尤其不能替代真实线上实验或直接调价依据。</p>
    </div>
  </>;
}
...
export default async function AnalyticsWorkbenchPage({ searchParams }: Props) {
  ...
  if (datasetContracts.length && !requestedContract) {
    ...
    if (params?.project_id) canonicalQuery.set('project_id', params.project_id);
  ...
  const tabs = (Object.entries(VIEW_LABELS) as Array<[Exclude<View, 'drilldown'>, string]>).map(([key, label]) => ({
    href: key === 'customers'
      ? customerHref(datasetId, 1, scopeDimension, scopeValue, customerSegment, projectId)
      : key === 'inventory'
        ? inventoryHref(datasetId, 1, itemScopeDimension, scopeValue, inventoryHealth, projectId)
        : key === 'replenishment'
          ? replenishmentHref(datasetId, 1, itemScopeDimension, scopeValue, replenishmentPriority, projectId)
      : key === 'elasticity'
          ? elasticityHref(datasetId, 1, itemScopeDimension, scopeValue, priceBand, projectId)
        : key === 'lifecycle'
          ? lifecycleHref(datasetId, 1, lifecycleStage, itemScopeDimension, scopeValue, projectId)
        : key === 'trend'
          ? trendHref(datasetId, scopeDimension, scopeValue, trendStart, trendEnd, projectId)
        : hrefFor(key, datasetId, scopeDimension, scopeValue, projectId),
    label,
    active: view === key,
  }));
  ...
  const drilldownBackHref = drilldownParentLabel
    ? hrefFor('overview', datasetId, itemScopeDimension, scopeValue, projectId)
    : params?.dimension && params?.value
      ? filteredOverviewHref(datasetId, params.dimension, params.value, projectId)
      : hrefFor('overview', datasetId, undefined, undefined, projectId);
  const trendBackHref = params?.dimension && params?.value
    ? drilldownHref(datasetId, params.dimension, params.value, itemScopeDimension, scopeValue, projectId)
    : hrefFor('overview', datasetId, scopeDimension, scopeValue, projectId);
  ...
  return (
    ...
    >
      ...
      {scopeDimension && scopeValue && view !== 'overview' && view !== 'drilldown' ? <div className="mb-5"><ScopeNotice dimension={scopeDimension} value={scopeValue} datasetId={datasetId} projectId={projectId} /></div> : null}
      ...
      {view === 'trend' ? (
        <div className="space-y-5">
          ...
          <TrendChart projectId={projectId} payload={trend} title={trendDimension && trendValue ? `筛选范围趋势：${DIMENSION_LABELS[trendDimension] || trendDimension} ${trendValue}` : '全店日趋势'} description="点击总览或明细趋势中的日期，可进入当天的趋势明细；右侧数字是真实数量。" />
      ...
      {view === 'overview' ? (
        <div className="space-y-5">
          {scopeDimension && scopeValue ? <ScopeNotice dimension={scopeDimension} value={scopeValue} datasetId={datasetId} projectId={projectId} /> : null}
      ...
      {view === 'channels' ? <Panel title={scopeDimension === 'item' || scopeDimension === 'category' ? '筛选范围的渠道销售' : '渠道与活动归因'} description={scopeDimension === 'item' || scopeDimension === 'category' ? '这里只按当前商品或类目关联的订单汇总渠道销售；数据没有商品级渠道曝光映射，因此不显示会话转化率。' : '订单转化率 = 订单数 ÷ 会话数；这是演示会话归因，不是平台广告归因。无父级筛选时，可以把渠道或活动作为新的总览范围。'}><DataTable><thead className="bg-muted/60"><tr className="text-left text-xs font-semibold text-muted-foreground"><th className="px-3 py-3">渠道</th><th className="px-3 py-3">活动</th><th className="px-3 py-3">渠道类型</th><th className="px-3 py-3">会话</th><th className="px-3 py-3">独立用户</th><th className="px-3 py-3">订单</th><th className="px-3 py-3">{scopeDimension === 'item' || scopeDimension === 'category' ? '统计说明' : '订单转化率'}</th><th className="px-3 py-3">净销售额</th><th className="px-3 py-3">查看范围</th><th className="px-3 py-3">查看明细</th></tr></thead><tbody className="divide-y divide-border/60">{asArray(channels?.metrics).map((row, index) => <tr key={`${String(row.channel_id)}-${String(row.campaign_id)}-${index}`} className="hover:bg-muted/35"><td className="px-3 py-3 text-sm">{!scopeDimension ? <Link className="text-primary hover:underline" href={filteredOverviewHref(datasetId, 'channel', row.channel_id, projectId)}>{text(row.channel_name)}</Link> : text(row.channel_name)}</td><td className="px-3 py-3 text-sm">{row.campaign_id !== undefined ? <Link className="text-primary hover:underline" href={drilldownHref(datasetId, 'campaign', row.campaign_id, itemScopeDimension, scopeValue, projectId)}>{text(row.campaign_name)}</Link> : text(row.campaign_name)}</td><td className="px-3 py-3 text-sm text-muted-foreground">{text(row.channel_type)}</td><td className="px-3 py-3 text-sm">{scopeDimension === 'item' || scopeDimension === 'category' ? '不提供' : displayNumber(row.sessions)}</td><td className="px-3 py-3 text-sm">{displayNumber(row.users)}</td><td className="px-3 py-3 text-sm">{displayNumber(row.orders)}</td><td className="px-3 py-3 text-sm">{scopeDimension === 'item' || scopeDimension === 'category' ? '按订单关联渠道统计' : displayPercent(row.order_conversion)}</td><td className="px-3 py-3 text-sm font-semibold">{displayMoney(row.net_sales)}</td><td className="px-3 py-3 text-sm">{!scopeDimension ? <div className="flex flex-col gap-1"><Link className="font-semibold text-primary hover:underline" href={filteredOverviewHref(datasetId, 'channel', row.channel_id, projectId)}>按渠道查看总览</Link>{row.campaign_id !== undefined ? <Link className="font-semibold text-primary hover:underline" href={filteredOverviewHref(datasetId, 'campaign', row.campaign_id, projectId)}>按活动查看总览</Link> : null}</div> : <span className="text-muted-foreground">已保留父级范围</span>}</td><td className="px-3 py-3 text-sm"><Link className="text-primary hover:underline" href={drilldownHref(datasetId, 'channel', row.channel_id, itemScopeDimension, scopeValue, projectId)}>查看渠道明细</Link></td></tr>)}</tbody></DataTable><div className="mt-4"><Limitations payload={channels} /></div></Panel> : null}

      {view === 'profit' ? <Panel title="毛利分析" description="毛利 = 销售额 - 成本；这里是演示数据估算，不是财务结算报表。点击渠道名称可按该渠道回到总览，或打开渠道明细。"><div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-5"><Metric label="销售额" value={displayMoney(asRecord(profit?.total)?.gross_sales)} hint="订单价格合计" /><Metric label="退款" value={displayMoney(asRecord(profit?.total)?.refunds)} hint="演示退款金额" /><Metric label="成本" value={displayMoney(asRecord(profit?.total)?.cost)} hint="演示商品成本" /><Metric label="毛利" value={displayMoney(asRecord(profit?.total)?.gross_profit)} hint="销售额减退款和成本" /><Metric label="毛利率" value={displayPercent(asRecord(profit?.total)?.gross_margin)} hint="毛利 ÷ 销售额" /></div><DataTable><thead className="bg-muted/60"><tr className="text-left text-xs font-semibold text-muted-foreground"><th className="px-3 py-3">渠道</th><th className="px-3 py-3">订单</th><th className="px-3 py-3">销售额</th><th className="px-3 py-3">退款</th><th className="px-3 py-3">成本</th><th className="px-3 py-3">毛利</th><th className="px-3 py-3">毛利率</th><th className="px-3 py-3">查看范围</th><th className="px-3 py-3">查看明细</th></tr></thead><tbody className="divide-y divide-border/60">{asArray(profit?.by_channel).map((row) => <tr key={String(row.channel_id)} className="hover:bg-muted/35"><td className="px-3 py-3 text-sm"><Link className="text-primary hover:underline" href={drilldownHref(datasetId, 'channel', row.channel_id, itemScopeDimension, scopeValue, projectId)}>{text(row.channel_name)}</Link></td><td className="px-3 py-3 text-sm">{displayNumber(row.orders)}</td><td className="px-3 py-3 text-sm">{displayMoney(row.gross_sales)}</td><td className="px-3 py-3 text-sm">{displayMoney(row.refunds)}</td><td className="px-3 py-3 text-sm">{displayMoney(row.cost)}</td><td className="px-3 py-3 text-sm font-semibold">{displayMoney(row.gross_profit)}</td><td className="px-3 py-3 text-sm">{displayPercent(row.gross_margin)}</td><td className="px-3 py-3 text-sm">{!scopeDimension ? <Link className="font-semibold text-primary hover:underline" href={filteredOverviewHref(datasetId, 'channel', row.channel_id, projectId)}>按渠道查看总览</Link> : <span className="text-muted-foreground">已保留父级范围</span>}</td><td className="px-3 py-3 text-sm"><Link className="text-primary hover:underline" href={drilldownHref(datasetId, 'channel', row.channel_id, itemScopeDimension, scopeValue, projectId)}>查看渠道明细</Link></td></tr>)}</tbody></DataTable><div className="mt-4"><Limitations payload={profit} /></div></Panel> : null}
      ...
      {view === 'replenishment' ? <Panel title="补货参考" description="按库存、日均销量和可解释的供货假设排序，帮助你先找出需要进一步核实的商品。点击上方判断可筛选对应商品。">
        <ReplenishmentSummary payload={replenishment} datasetId={datasetId} filterDimension={itemScopeDimension} filterValue={scopeValue} selectedPriority={replenishmentPriority} projectId={projectId} />
