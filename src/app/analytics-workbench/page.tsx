...
function ElasticitySummary({ payload, datasetId, filterDimension, filterValue, priceBand, projectId }: { payload: JsonRecord | null; datasetId: string; filterDimension?: string; filterValue?: string; priceBand?: string; projectId?: string }) {
  ...
  return <>
    ...
    <div className={`mt-5 rounded-xl border p-4 text-sm leading-6 ${experimentReady ? 'border-sky-200 bg-sky-50 text-sky-950' : 'border-slate-200 bg-slate-50 text-slate-800'}`}>
      ...
      {experimentReady && experimentRows.length > 0 ? <DataTable><thead className="bg-white/70"><tr className="text-left text-xs font-semibold text-muted-foreground"><th className="px-3 py-3">实验</th><th className="px-3 py-3">对照价格</th><th className="px-3 py-3">处理价格</th><th className="px-3 py-3">对照购买率</th><th className="px-3 py-3">处理购买率</th><th className="px-3 py-3">购买率变化</th><th className="px-3 py-3">变化范围（95%）</th><th className="px-3 py-3">样本判断</th></tr></thead><tbody className="divide-y divide-border/60">{experimentRows.map((row) => { const status = text(row.significance_status); const label = status === 'significant' ? '样本显示有差异' : status === 'small_sample' ? '样本较少，暂不判断' : '暂未发现明确差异'; return <tr key={String(row.experiment_id)}><td className="px-3 py-3 text-sm">{text(row.experiment_id)}</td><td className="px-3 py-3 text-sm">{displayMoney(row.control_price)}</td><td className="px-3 py-3 text-sm">{displayMoney(row.treatment_price)}</td><td className="px-3 py-3 text-sm">{displayPercent(row.control_conversion_rate)}</td><td className="px-3 py-3 text-sm">{displayPercent(row.treatment_conversion_rate)}</td><td className="px-3 py-3 text-sm font-semibold">{displayPercent(row.absolute_conversion_lift)}</td><td className="px-3 py-3 text-sm">{displayPercent(row.absolute_conversion_lift_ci_low)} ～ {displayPercent(row.absolute_conversion_lift_ci_high)}</td><td className="px-3 py-3 text-sm"><span>{label}</span><br /><span className="text-xs text-muted-foreground">p 值 {row.p_value == null ? '—' : text(row.p_value)}</span></td></tr>; })}</tbody></DataTable> : <p className="mt-2 text-xs">需要同时具备对照组、处理组、每组曝光人数、每组购买人数和分组方式，才会展示实验对比。</p>}
