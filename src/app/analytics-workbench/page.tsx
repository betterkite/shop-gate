    ...
    >
      ...
      {view === 'overview' ? (
        <div className="space-y-5">
          ...
          <div className="grid gap-5 lg:grid-cols-2"><Panel title="商品阶段" description="根据窗口内购买活跃度推断，不等于真实上架/下架生命周期。"><div className="grid grid-cols-2 gap-2">{Object.entries(asRecord(lifecycle?.stage_counts) ?? {}).map(([label, value]) => <Link key={label} href={hrefFor('lifecycle', datasetId)} className="flex items-center justify-between rounded-lg bg-muted/35 px-3 py-2 text-sm hover:bg-muted"><span>{label}</span><strong>{displayNumber(value)}</strong></Link>)}</div></Panel><Panel title="价格弹性边界" description="当前每个商品只有一个演示价格，无法判断调价后销量会怎样变化。"><Link href={hrefFor('elasticity', datasetId)} className="block rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 hover:border-amber-400">{text(elasticity?.explanation)}<span className="mt-2 block text-xs font-semibold">查看价格带对比 →</span></Link></Panel></div>
