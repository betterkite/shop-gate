...
function SyntheticBadge() {
  return (
    <span
      ...
      title="价格/库存/品牌/店铺与成交总额（GMV）来自合成主数据，不代表真实交易数据"
    >
      合成口径
    </span>
  );
  ...
  return (
    ...
    >
      <section className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-6">
        <Metric label="成交总额（GMV）" value={displayMoney(totals?.gmv)} hint="金额为合成口径" />
        ...
        <Metric label="购买转化率" value={displayPercent(data.summary?.buy_conversion)} hint="购买事件 / 页面浏览量" />
      ...
      {!data.apiEnabled ? (
        ...
        <section className="space-y-4">
          ...
          <p className="text-xs text-muted-foreground">类目名为合成映射；成交总额（GMV）= 购买事件 × 合成价格。</p>
        </section>
      ) : (
        <section className="space-y-4">
          <div><h2 className="text-lg font-semibold">渠道贡献与店铺层级</h2><p className="mt-1 text-sm text-muted-foreground">比较不同店铺 tier 的流量、购买和成交总额（GMV）贡献。</p></div>
