...
export default async function CommercePlatformPage({ searchParams }: Props) {
  ...
  return (
    <RetailPageShell
      title="商品运营"
      subtitle={`窗口 ${win.start} ~ ${win.end} · 行为源 ${behaviorSource} · ${data.behaviorSource === 'synthetic' ? '合成演示数据' : '真实行为流'}`}
      badge={<SyntheticBadge />}
      tabs={tabs}
    >
      <section className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-5">
        <Metric label="窗口 GMV" value={displayMoney(totals?.gmv)} hint="金额为合成口径" />
        ...
        <Metric label="购买转化" value={displayPercent(data.summary?.buy_conversion)} hint="buy / pv" />
        <Metric label="客单价" value={displayMoney(data.summary?.avg_price)} hint="合成价格口径" />
      </section>
      ...
      {!data.apiEnabled ? (
        ...
        <section className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">先定位 GMV/流量贡献，再结合库存与转化决定动作。</p>
            </div>
          ...
          <DataTable>
            <thead className="bg-muted/60">
              <tr className="text-left text-xs font-semibold text-muted-foreground">
                {['商品', '类目', '店铺', '渠道', '价格', '库存', '曝光', '购买', 'GMV', '转化'].map((h) => <th key={h} className="whitespace-nowrap px-3 py-3">{h}</th>)}
              </tr>
            </thead>
        ...
        <section className="space-y-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">用 GMV、流量和转化同时看结构，避免只追逐单一规模指标。</p>
          </div>
          <DataTable>
            <thead className="bg-muted/60"><tr className="text-left text-xs font-semibold text-muted-foreground">{['#', '类目', '曝光', '购买', 'GMV', '转化率', '客单价'].map((h) => <th key={h} className="whitespace-nowrap px-3 py-3">{h}</th>)}</tr></thead>
          ...
          <p className="text-xs text-muted-foreground">类目名为合成映射；GMV = 购买事件 × 合成价格。</p>
        </section>
      ) : (
        <section className="space-y-4">
          <div><h2 className="text-lg font-semibold">渠道贡献与店铺层级</h2><p className="mt-1 text-sm text-muted-foreground">比较不同店铺 tier 的流量、购买和 GMV 贡献。</p></div>
          <div className="grid gap-4 md:grid-cols-2">
            {data.channels.map((channel) => {
              ...
              return <article key={String(channel.channel)} className="rounded-2xl border border-border/70 bg-card p-5 shadow-sm"><div className="flex items-center justify-between gap-3"><h3 className="font-semibold">{String(channel.channel ?? '-')}</h3><span className="text-xs text-muted-foreground">{displayPercent(gmvShare)} GMV 占比</span></div><div className="my-4 h-2 rounded-full bg-muted"><div className="h-2 rounded-full bg-primary" style={{ width: `${Math.max(1, gmvShare * 100).toFixed(1)}%` }} /></div><dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm"><dt className="text-muted-foreground">店铺数</dt><dd className="text-right">{displayNumber(channel.shop_count, 0)}</dd><dt className="text-muted-foreground">商品数</dt><dd className="text-right">{displayNumber(channel.item_count, 0)}</dd><dt className="text-muted-foreground">曝光</dt><dd className="text-right">{displayNumber(channel.pv, 0)}</dd><dt className="text-muted-foreground">购买</dt><dd className="text-right">{displayNumber(channel.buy, 0)}</dd><dt className="text-muted-foreground">GMV</dt><dd className="text-right font-semibold">{displayMoney(channel.gmv)}</dd></dl></article>;
            })}
          </div>
          <p className="text-xs text-muted-foreground">渠道 = 店铺 tier（standard / premium / flagship）；事件为真实 UserBehavior，GMV 为合成口径。</p>
        </section>
      )}
    </RetailPageShell>
  );
