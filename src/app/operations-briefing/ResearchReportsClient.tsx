    ...
    <article className="overflow-hidden rounded-2xl border border-border/60 bg-card shadow-[0_24px_54px_-40px_hsl(var(--shadow-color)/0.6)]">
      ...
      <div className="grid gap-5 p-5 sm:p-6 lg:grid-cols-[minmax(0,1fr)_300px]">
        <div>
          ...
          <div className="mt-5 grid grid-cols-3 gap-2"><div className="rounded-lg bg-muted/45 p-3"><p className="text-[10px] text-muted-foreground">候选标的</p><p className="mt-1 text-lg font-black text-foreground">{candidates.length}</p></div><div className="rounded-lg bg-muted/45 p-3"><p className="text-[10px] text-muted-foreground">观察池覆盖</p><p className="mt-1 text-lg font-black text-foreground">{coverage ? `${Math.round(coverage.coverageRatio * 100)}%` : "-"}</p></div><div className="rounded-lg bg-muted/45 p-3"><p className="text-[10px] text-muted-foreground">复核事项</p><p className="mt-1 text-lg font-black text-foreground">{risks.length}</p></div></div>
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      {latest ? <section className="space-y-4"><ResearchSectionHeader eyebrow="LATEST BRIEF" title="最新研究摘要" description="优先展示最新评分、建议、候选、覆盖率和证据状态。" action={<Button variant="ghost" size="sm" onClick={() => onViewChange("reports")}>查看全部报告 <ArrowRight className="h-3.5 w-3.5" /></Button>} /><LatestResearchCard report={latest} onOpenReports={() => onViewChange("reports")} onSend={() => onSend(latest.id)} isSending={isSending} /></section> : (
        <section className="grid items-stretch gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)]"><div className="rounded-2xl border border-dashed border-primary/25 bg-card px-6 py-10 text-center sm:py-12"><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10 text-primary"><Sparkles className="h-5 w-5" /></span><h3 className="mt-4 text-xl font-black text-foreground">首份日报会沉淀哪些内容？</h3><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-muted-foreground">重点商品、观察池覆盖、综合评分、风险等级、经营建议、证据状态与人工复核清单。</p><Button variant="outline" className="mt-4" onClick={() => onViewChange("automation")}>检查生成条件<ArrowRight className="h-4 w-4" /></Button></div><div className="rounded-2xl border border-border/60 bg-card p-5"><h3 className="text-base font-bold text-foreground">当前研究边界</h3><div className="mt-4 space-y-3">{["使用本地 commerce-data 与商品池事实，不拼装未知外部 API。", "ClickHouse 不可用时保留 TimescaleDB 稳定回退路径。", "新闻与舆情源尚未启用，不会伪造事件结论。", "日报只作为研究与复核材料，不输出确定性买卖指令。"].map((item) => <div key={item} className="flex items-start gap-2 text-sm leading-6 text-muted-foreground"><ShieldAlert className="mt-1 h-4 w-4 shrink-0 text-amber-500" />{item}</div>)}</div></div></section>
      )}

      <section className="grid gap-5 xl:grid-cols-2">
        <div className="space-y-4"><ResearchSectionHeader eyebrow="WATCHLIST" title="研究范围" description="当前日报所覆盖的观察池、商品、渠道与计划。" /><div className="space-y-3">{data.watchlists.map((watchlist) => <article key={watchlist.id} className="rounded-xl border border-border/60 bg-card p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="font-bold text-foreground">{watchlist.name}</h3><p className="mt-1 text-xs text-muted-foreground">{watchlist.universeId ?? "未绑定观察池"}</p></div><ResearchStatusBadge status={watchlist.status} /></div><p className="mt-3 text-sm leading-6 text-muted-foreground">{watchlist.description}</p><div className="mt-4 flex flex-wrap gap-1.5">{watchlist.symbols.map((symbol) => <Badge key={symbol} variant="outline" className="font-mono text-[10px]">{symbol}</Badge>)}{watchlist.markets.map((market) => <Badge key={market} className="bg-primary/10 text-[10px] text-primary hover:bg-primary/10">{market}</Badge>)}</div></article>)}</div></div>
