        ...
        <section className="space-y-6">
          ...
          <div className="grid gap-4 lg:grid-cols-[1.25fr_0.75fr]">
            <section className="min-w-0 rounded-2xl border border-border/70 bg-card p-5 shadow-sm">
              <div className="mb-4 flex items-center justify-between"><div><h2 className="font-semibold">异常与趋势信号</h2><p className="mt-1 text-xs text-muted-foreground">比较今天和前一天，找出需要继续查看的变化。</p></div><span className="rounded-full bg-muted px-2.5 py-1 text-xs text-muted-foreground">{String(data.daily?.status ?? 'ok')}</span></div>
              <DataTable><thead className="bg-muted/60"><tr className="text-left text-xs font-semibold text-muted-foreground"><th className="px-3 py-3">指标</th><th className="px-3 py-3">日环比</th><th className="px-3 py-3">判断</th></tr></thead><tbody className="divide-y divide-border/60">{dayOverDay ? Object.entries(dayOverDay).map(([key, value]) => { const n = Number(value); const label = n < 0 ? '比前一天少，建议查看原因' : n > 0 ? '比前一天好' : '和前一天差不多'; return <tr key={`dod-${key}`} className="hover:bg-muted/35"><Cell value={METRIC_LABELS[key] ?? key} className="font-medium" /><Cell value={value === null ? '-' : displayPercent(value)} /><Cell value={label} className={n < 0 ? 'text-rose-600' : 'text-emerald-600'} /></tr>; }) : <tr><Cell value="第一天没有前一天数据，暂时无法比较。" /></tr>}</tbody></DataTable>
            </section>
