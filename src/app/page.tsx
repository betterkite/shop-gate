...
const RESEARCH_STARTERS: Array<{
...
}> = [
  {
    ...
    label: "加购后未购买",
    prompt: "按当前数据集全库、最近 9 天，从浏览、收藏、加购到购买的转化情况怎样？哪个环节流失最多，集中在哪些类目？生成经营看板。",
  },
  {
    capabilityId: "catalog_structure",
    label: "类目销售额结构",
    prompt: "按当前数据集全库、最近 9 天，成交总额（GMV）最高的 5 个类目是哪些？给出各类目的购买转化率和平均每次购买金额对比，生成经营看板。",
  },
  {
    capabilityId: "price_inventory",
    label: "库存积压商品清单",
    prompt: "按当前数据集全库、最近 9 天，库存相对销量偏高的 10 个商品是哪些？它们的页面浏览量和购买转化率如何？生成经营看板。",
  },
];
...
export default function HomePage() {
  ...
  const openRecentResults = useCallback(() => {
    const section = document.getElementById("recent-results");
    if (section) {
      section.scrollIntoView({ behavior: "smooth", block: "start" });
      window.history.replaceState(null, "", "#recent-results");
      return;
    }
    router.push("/#recent-results");
  }, [router]);
  ...
  return (
    <div className="home-shell relative flex min-h-screen flex-col overflow-x-clip bg-background text-foreground">
      <header className="platform-header sticky top-0 z-40 flex h-16 shrink-0 items-center justify-between px-3 md:px-6">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <nav className="ml-4 hidden items-center gap-1 lg:flex" aria-label="首页导航">
            ...
            <Button type="button" variant="ghost" size="sm" onClick={openRecentResults} className="h-11 gap-2 rounded-none px-3 text-xs font-semibold text-muted-foreground">
          ...
          {recentResults.length > 0 ? (
            ...
            >
              <div className="flex items-end justify-between gap-3">
                ...
                <Button type="button" variant="ghost" size="sm" onClick={openRecentResults} className="min-h-11 gap-1 rounded-none border-b border-border px-1 text-xs">查看成果<ChevronRight className="h-3.5 w-3.5" /></Button>
              </div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 grid h-[calc(4rem+env(safe-area-inset-bottom))] grid-cols-4 border-t border-border/80 bg-background/95 px-2 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl md:hidden" aria-label="移动端首页导航">
        ...
        <button type="button" onClick={openRecentResults} className="flex min-h-11 flex-col items-center justify-center gap-0.5 border-t-2 border-transparent text-[10px] font-semibold text-muted-foreground"><FileChartColumn className="h-4 w-4" />成果</button>
