...
describe('API route authorization coverage', () => {
  ...
  it('maps high-risk global and cross-project routes to catalogued capabilities', async () => {
    const expectedActions: Record<string, string[]> = {
      ...
      'src/app/api/settings/cli-status/route.ts': ['commerce.data.read'],
      'src/app/api/settings/global/route.ts': ['commerce.data.read', 'platform.settings.manage'],
      'src/app/api/skills/[skillId]/package/route.ts': ['commerce.data.read'],
      'src/app/api/skills/route.ts': ['commerce.data.read', 'platform.settings.manage'],
