...
const FALLBACK_DATA_PROVIDERS: CapabilityCenterDataProvider[] = [
  ...
  },
  {
    ...
    limitations: ['实时行情、分红和公告可用；历史 K 线 push2his 当前不可达。'],
  },
  {
    ...
    status: 'available',
    description: '腾讯公开 K 线端点当前探针可通，适合兜底 OHLCV 样本。',
    endpoints: ['https://web.ifzq.gtimg.cn/appstock/app/fqkline/get'],
    cacheTtlSeconds: 3_600,
    limitations: ['成交额、换手率通常为空，不适合作为高质量历史主源。'],
  },
  {
    id: 'ths-public-kline',
    name: '同花顺公开 K 线端点',
    ...
    status: 'available',
    description: '同花顺网页历史线数据端点当前探针可通，可作为 A 股历史 K 线候选源继续解析验证。',
    endpoints: ['https://d.10jqka.com.cn/v6/line/hs_{symbol}/01/all.js'],
    cacheTtlSeconds: 3_600,
    limitations: ['网页公开端点不是正式 SLA，返回为压缩 JavaScript，需要单独解析和字段校验。'],
  },
  {
    ...
    endpoints: ['/api/v1/ingestion/baostock/history', 'Python SDK: baostock.query_history_k_data_plus'],
  ...
  },
  {
    ...
    category: 'enrichment-provider',
    status: 'degraded',
    description: 'Python 聚合数据源，当前用于补 A 股历史 K 线的成交额、振幅、涨跌额和换手率。',
  ...
  },
  {
    ...
    category: 'fallback-provider',
    status: 'available',
    description: 'Yahoo Finance Chart API 当前探针可通，适合海外股票、ETF 和指数历史行情。',
    endpoints: ['https://query1.finance.yahoo.com/v8/finance/chart/{symbol}'],
    cacheTtlSeconds: 3_600,
    limitations: ['主要用于海外市场，不作为 A 股历史行情主源。'],
  },
  {
    ...
    status: 'planned',
    description: '同花顺正式数据接口，覆盖历史行情、基本面和指标数据，通常需要 iFinD 终端或授权账号。',
