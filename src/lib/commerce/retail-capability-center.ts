...
const FALLBACK_DATA_PROVIDERS: CapabilityCenterDataProvider[] = [
  {
    ...
    category: 'dataset',
    status: 'available',
    description: '真实行为流的窗口、事件量、用户/商品/类目规模与来源计数。',
    endpoints: ['/api/v1/commerce/meta'],
    cacheTtlSeconds: 300,
    limitations: ['窗口外趋势不支持；首日无环比。'],
  },
  {
    ...
    status: 'available',
    description: '真实用户行为流（pv/fav/cart/buy），按用户抽样导入 TimescaleDB。',
    endpoints: ['commerce.user_behavior_events'],
    cacheTtlSeconds: null,
    limitations: ['口径偏用户行为，无订单/支付字段；GMV 由 buy × 合成价格构成。'],
  },
  {
    ...
    status: 'available',
    description: '价格/库存/品牌/店铺为确定性合成档案；类目/商品 ID 继承真实行为流。',
    endpoints: ['commerce.items', 'commerce.categories', 'commerce.brands', 'commerce.shops'],
    cacheTtlSeconds: null,
    limitations: ['合成口径，展示必须带徽标；不代表真实交易数据。'],
  },
  {
    ...
    cacheTtlSeconds: null,
    limitations: ['聚合依赖商品主数据价格，属合成金额口径。'],
  },
  {
