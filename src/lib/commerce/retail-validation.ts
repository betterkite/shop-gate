...
export type RetailValidationCheckStatus = 'passed' | 'failed' | 'warning';
export type RetailValidationStatus = 'passed' | 'failed';

export interface RetailValidationCheck {
  id: string;
  name: string;
  status: RetailValidationCheckStatus;
  summary: string;
  details?: string;
  durationMs?: number;
...
export type RetailValidationStaleReason =
  | 'artifact_modified_after_report'
  | 'run_id_mismatch';

export interface RetailValidationArtifactMtime {
  path: string;
  mtimeMs: number;
}

export interface RetailValidationFreshness {
  stale: boolean;
  reasons: RetailValidationStaleReason[];
 ...
 */
export function assessRetailValidationReportFreshness(params: {
  reportRunId?: string | null;
  currentRunId?: string | null;
  reportMtimeMs: number;
  artifacts: RetailValidationArtifactMtime[];
}): RetailValidationFreshness {
  ...
  const reasons: RetailValidationStaleReason[] = [];
...
export interface RetailValidationRepairStep {
  checkId: string;
  checkName: string;
  summary: string;
  actions: string[];
...
export interface RetailValidationRepairPlan {
  schemaVersion: 1;
  status: 'needed';
  projectId: string;
  reportPath: string;
  repairPlanPath: string;
  steps: RetailValidationRepairStep[];
  createdAt: string;
}

export interface RetailDashboardTemplateRestoreResult {
  restored: boolean;
  reason: string;
...
export interface PrepareRetailProjectForValidationParams {
  projectId: string;
  projectPath: string;
}
...
async function safeRunCheck(
  id: string,
  name: string,
  checker: () => Promise<Omit<RetailValidationCheck, 'id' | 'name' | 'durationMs'>>
): Promise<RetailValidationCheck> {
  const startedAt = Date.now();
...
async function checkBuild(projectPath: string): Promise<Omit<RetailValidationCheck, 'id' | 'name' | 'durationMs'>> {
...
export async function checkRetailArtifactPolicy(projectPath: string): Promise<RetailValidationCheck> {
...
async function ensurePrefetchedFinalData(projectPath: string) {
  ...
  try {
    await prefetchRetailDataForRunPlan({
      projectPath,
      plan: runPlan as unknown as RetailRunPlan,
    });
  } catch (error) {
    console.warn(
...
async function writeValidationReport(projectPath: string, report: RetailValidationReport) {
  await ensureRetailWorkspace(projectPath);
  await fs.writeFile(validationReportPath(projectPath), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
}

function actionsForFailedCheck(check: RetailValidationCheck): string[] {
  switch (check.id) {
    case 'next_build':
      return [
 ...
 */
export function retailValidationRepairWritableGlobs(
...
function targetedReadsForFailedChecks(failedChecks: RetailValidationCheck[]): string[] {
  const paths = new Set<string>([
...
function completionConditionForFailedCheck(check: RetailValidationCheck): string {
  switch (check.id) {
    case 'next_build':
...
export function buildRetailValidationRepairPlan(report: RetailValidationReport): RetailValidationRepairPlan {
...
async function writeValidationRepairPlan(projectPath: string, report: RetailValidationReport) {
...
function buildValidationSummary(report: RetailValidationReport): string {
...
export async function repairRetailPlatformOwnedArtifacts(params: {
  projectPath: string;
  requestId: string;
...
export function isRetailDashboardTemplateRecoveryEligible(
  report: RetailValidationReport,
): boolean {
...
export function buildRetailValidationRepairInstruction(
  report: RetailValidationReport,
  options: { originalInstruction?: string } = {}
): string {
  const failedChecks = report.checks.filter((check) => check.status === 'failed');
  const repairPlan = buildRetailValidationRepairPlan(report);
 ...
 */
export async function prepareRetailProjectForValidation(
  params: PrepareRetailProjectForValidationParams,
): Promise<string> {
  const projectPath = path.resolve(/*turbopackIgnore: true*/ params.projectPath);
...
export async function readRetailValidationReport(projectPath: string): Promise<RetailValidationReport | null> {
  ...
  try {
    const parsed = JSON.parse(report) as RetailValidationReport;
    ...
    if (reportStat) {
      const freshness = assessRetailValidationReportFreshness({
        ...
        artifacts: artifactStats.filter(
          (artifact): artifact is RetailValidationArtifactMtime => artifact !== null,
        ),
      });
      if (freshness.stale) {
...
export async function readRetailValidationRepairPlan(projectPath: string): Promise<RetailValidationRepairPlan | null> {
  ...
  try {
    const parsed = JSON.parse(report) as RetailValidationRepairPlan;
