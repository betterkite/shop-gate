...
import {
  projectFinancePlanToDataAgentPlan,
  projectFinanceProfileSelection,
  projectFinanceRewriteToDataAgentTask,
} from '@/lib/domains/finance/data-agent-projection';
...
export async function readQuantRunPlan(projectPath: string): Promise<QuantRunPlan | null> {
  return readJsonFile<QuantRunPlan>(path.join(projectPath, FINANCE_RUN_PLAN_RELATIVE_PATH));
}
...
export async function writeInitialRunPlan(params: {
  ...
  const dataAgentTask = projectFinanceRewriteToDataAgentTask(queryRewrite);
  const dataAgentPlan = projectFinancePlanToDataAgentPlan(plan);
  ...
  const dataAgentProfile = projectFinanceProfileSelection(
