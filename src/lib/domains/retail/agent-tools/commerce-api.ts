...
const ALLOWED_COMMERCE_API_PATHS = [
  ...
  /^\/api\/v1\/commerce\/items\/[^/]+\/daily$/,
  /^\/api\/v1\/commerce\/inventory-risk$/,
  /^\/api\/v1\/commerce\/summary$/,
  /^\/api\/v1\/commerce\/capabilities$/,
  /^\/api\/v1\/commerce\/analytics\/(overview|trend|rfm|retention|channel-campaign|profit|inventory|replenishment|lifecycle|price-elasticity|drilldown)$/,
] as const;
...
interface CommerceJsonOmission {
  path: string;
  kind: 'array' | 'string';
  originalSize: number;
  retainedSize: number;
...
interface CommerceJsonProjectionState {
  omissionCount: number;
  omissions: CommerceJsonOmission[];
}
...
export interface RetailApiGetInput {
  path: string;
  query: Record<string, QueryValue>;
}

export interface PiAgentRetailApiToolOptions {
  timeoutMs?: number;
...
function parseRetailApiGetInput(value: unknown): RetailApiGetInput {
...
function buildCommerceApiUrl(apiPath: string, query: Record<string, QueryValue>): URL {
  if (!apiPath.startsWith(COMMERCE_API_PREFIX) || apiPath.startsWith('//')) {
    throw new PiAgentToolError(
      'COMMERCE_API_PATH_DENIED',
      'commerce_api_get only accepts paths beginning with /api/v1/.',
    );
  }
  if (apiPath.includes('\\') || apiPath.includes('?') || apiPath.includes('#') || /[\r\n\0]/.test(apiPath)) {
    throw new PiAgentToolError('COMMERCE_API_PATH_DENIED', 'The API path must not contain query text, fragments, backslashes, or control characters.');
  }
  ...
  for (const rawSegment of rawSegments) {
    ...
    try {
      ...
      throw new PiAgentToolError('COMMERCE_API_PATH_DENIED', 'The API path contains malformed percent encoding.');
    }
    if (decoded === '.' || decoded === '..' || decoded.includes('/') || decoded.includes('\\')) {
      throw new PiAgentToolError('COMMERCE_API_PATH_DENIED', 'API path traversal and encoded separators are denied.');
    }
  }
  ...
  if (!ALLOWED_COMMERCE_API_PATHS.some((pattern) => pattern.test(url.pathname))) {
    throw new PiAgentToolError(
      'COMMERCE_API_ENDPOINT_DENIED',
      'The requested local API endpoint is not in the PI Agent read-only retail commerce allowlist.',
    );
  }
  ...
  if (url.toString().length > MAX_URL_CHARS) {
    throw new PiAgentToolError('COMMERCE_API_URL_TOO_LONG', `Commerce API URLs cannot exceed ${MAX_URL_CHARS} characters.`);
  }
  return url;
}
...
function recordOmission(
  state: CommerceJsonProjectionState,
  omission: CommerceJsonOmission,
): void {
...
function serializeBoundedPreview(
  ...
  const minimal = JSON.stringify({
    $piAgent: {
      ...
      version: COMMERCE_WINDOW_VERSION,
      truncated: true,
    },
  });
...
/**
 * 让大的 commerce-data 响应保持合法 JSON。时序数组保留早期采样与更大的
 * 近端尾部；元数据、汇总、数据质量字段和所有对象字段对模型保持可见。
 */
function compactCommerceOutput(
  value: string,
  options: {
    limit: number;
  ...
  if (!options.responseByteLimitReached) {
    try {
      ...
      for (const projection of projections) {
        const state: CommerceJsonProjectionState = { omissionCount: 0, omissions: [] };
      ...
      // API 声明是 JSON，但有界的合法预览比把超长或畸形响应泄漏进模型
      // 上下文更安全。
    }
  }
  ...
  return {
    text: serializeBoundedPreview(value, options.limit, {
      ...
      retryHint: options.responseByteLimitReached
        ? 'Retry with a narrower date range or smaller limit.'
...
export function createCommerceApiGetTool(options: PiAgentRetailApiToolOptions = {}): PiAgentTool<RetailApiGetInput> {
  ...
  return {
    ...
    parseInput: parseRetailApiGetInput,
    execute: (input, context) => executePiAgentTool(context.signal, timeoutMs, async (signal) => {
      requestCount += 1;
      if (requestCount > maxRequests) {
        throw new PiAgentToolError(
          'COMMERCE_API_REQUEST_BUDGET_EXCEEDED',
          `This PI Agent run exceeded its ${maxRequests}-request commerce API budget.`,
        );
      }
      const url = buildCommerceApiUrl(input.path, input.query);
      ...
      const output = compactCommerceOutput(body.text, {
      ...
      if (!response.ok) {
        return {
          ok: false,
          error: {
            code: 'COMMERCE_API_HTTP_ERROR',
            message: `Local commerce API returned HTTP ${response.status}.`,
            details: {
