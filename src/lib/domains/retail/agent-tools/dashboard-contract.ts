...
function summarizeRunPlan(file: InspectedFile): JsonRecord {
  ...
  return {
    ...
    plannedEntities: (() => {
      const entities = isRecord(plan?.plannedEntities) ? plan.plannedEntities : null;
      const categoryIds = Array.isArray(entities?.categoryIds) ? entities.categoryIds : [];
      const itemIds = Array.isArray(entities?.itemIds) ? entities.itemIds : [];
      return {
        categoryCount: categoryIds.length,
        itemCount: itemIds.length,
        sampledCategories: boundedStrings(categoryIds.slice(0, 8), 8, 16),
        wholeCatalog: categoryIds.length === 0 && itemIds.length === 0,
      };
    })(),
...
function summarizeFinalData(file: InspectedFile): JsonRecord {
  ...
  return {
    ...
    hasWindow: isRecord(value?.window),
    plannedEntities: (() => {
      const entities = isRecord(value?.plannedEntities) ? value.plannedEntities : null;
      const categoryIds = Array.isArray(entities?.categoryIds) ? entities.categoryIds : [];
      const itemIds = Array.isArray(entities?.itemIds) ? entities.itemIds : [];
      return { categoryCount: categoryIds.length, itemCount: itemIds.length };
    })(),
    funnelStageCount: nestedArrayLength(value, "funnel", "stages"),
    categoriesRowCount: (() => {
      const datasets = isRecord(value?.datasets) ? value.datasets : null;
      const categories = isRecord(datasets?.categories) ? datasets.categories : null;
      return Array.isArray(categories?.rows) ? categories.rows.length : 0;
    })(),
    syntheticDisclosed:
      JSON.stringify(parsed.record ?? {}).includes("synthetic"),
...
function buildPageSignals(page: InspectedFile): JsonRecord {
  ...
  const chartDataLines = matchingLines(
    lines,
    /getFunnel\b|getCategories\b|getItemSeries\b|getInventoryRows\b|getDailySummary\b|dataset\.(?:funnel|categories|summary)|sellThrough/i,
  );
...
function buildVisualSourceMap(
  ...
  const markerLine = matchingLines(
    pageLines,
    /data-visual-language=["']retail-workbench["']/,
    1,
  )[0] ?? null;
  ...
  return {
    ...
    preparedVisualEdit: {
      ...
      objective:
        "Use one continuous retail-workbench canvas with hairline section dividers, contiguous metric strips, dominant charts/matrices, and dense tables; remove repeated rounded/shadowed surfaces.",
      pageMarker: markerLine
        ? { present: true, line: markerLine }
        : {
            present: false,
            targetRange: rootMainLine ? [rootMainLine, rootMainLine] : null,
            action: 'Add data-visual-language="retail-workbench" to the root <main>.',
          },
      cssTargetSelectors: cardSurfaceCandidates.map((item) => item.selector),
      cssRecipe: [
        "Scope the override to the root retail-workbench marker.",
  ...
  return {
    ...
    task: compactTask
      ? {
          ...
          plannedEntities: compactTask.plannedEntities,
