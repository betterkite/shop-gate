...
export const PRODUCT_SCREENSHOT_FIELDS = [
  'window_note',
  'items[].name_or_title',
...
const DEFAULT_ATTACHMENT_CONTEXT = '.data-agent/attachments.json';
...
function metadataPayload(params: {
  ...
  return {
    ...
    imageExtraction: {
      ...
      needs_manual_confirmation: true,
      manual_confirmation_fields: PRODUCT_SCREENSHOT_FIELDS,
    },
    dashboardContract: {
      requiredFinalDataFields: ['window', 'plannedEntities', 'datasets', 'imageExtraction'],
      evidenceFiles: [
...
export async function extractUploadedImageMetadata(
  ...
  if (input.imagePath) {
    const requestedImagePath = pathForPolicy(input.imagePath);
    attachments = [{ id: 'image-1', name: path.basename(input.imagePath), path: requestedImagePath }];
    contextPath = pathForPolicy(requestedContextPath).replaceAll(path.sep, '/');
  } else {
...
export function createImageExtractionTool(
  ...
  return {
    ...
    description:
      'Read Shop Gate uploaded-image attachments, verify the files inside the PI Agent workspace, and return the product/inventory screenshot metadata/extraction contract. This tool performs no OCR and never invents uncertain fields.',
    effect: 'read',
    ...
    inputSchema: {
      ...
      properties: {
        attachmentContextPath: {
          type: 'string',
          description: 'Workspace-relative attachment manifest path; defaults to .data-agent/attachments.json.',
        },
        imagePath: {
          type: 'string',
    ...
    execute: (input, context) => executePiAgentTool(
      ...
      async (signal) => {
        ...
        const content = data.status === 'metadata_ready'
          ? `PI Agent verified ${data.images.length} uploaded image(s). OCR is disabled; uncertain product/inventory fields require manual confirmation.`
          : data.message;
        return {
          ok: true,
          data,
