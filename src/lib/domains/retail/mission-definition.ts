...
export const RETAIL_REQUIRED_VALIDATION_CHECK_IDS = [
  'artifact_policy',
  'next_build',
  'preview_http_200',
  'visual_presentation',
...
export const RETAIL_ALLOWED_VALIDATION_WARNINGS = ['evidence_files'] as const;
  ...
  return [
    {
      ...
      inputArtifacts: ['.data-agent/retail-query-rewrite.json'],
      outputArtifacts: [
        '.data-agent/task.json',
        '.data-agent/plan.json',
        '.data-agent/retail-run-plan.json',
    ...
    },
    {
      ...
      allowedTools: [],
      // v1 领域 skill 尚未发布（P8）：数据取数口径由 capability 的
      // promptGuidance/validationRules 与 commerce-data 端点合同承载。
      requiredSkillSections: ['query-rewrite', 'data-quality'],
      inputArtifacts: [
        '.data-agent/task.json',
        '.data-agent/plan.json',
        '.data-agent/retail-query-rewrite.json',
...
export function createRetailMissionDefinition(input: {
  maxRepairAttempts: number;
  expectedArtifacts?: readonly string[];
}): PiAgentMissionDefinition {
  return {
    id: 'retail.core-workspace',
    version: '1.0.0',
    validationReportPath: '.data-agent/validation.json',
    artifacts: artifacts(input.expectedArtifacts ?? []),
    requiredValidationCheckIds: [...RETAIL_REQUIRED_VALIDATION_CHECK_IDS],
    allowedValidationWarnings: [...RETAIL_ALLOWED_VALIDATION_WARNINGS],
    ...
    acceptancePredicates: [
      { id: 'candidate_submission', kind: 'candidate_submission', required: true },
      {
        ...
        required: true,
        parameters: { checkIds: [...RETAIL_REQUIRED_VALIDATION_CHECK_IDS] },
      },
