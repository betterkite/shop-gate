...
export const RETAIL_VISUALIZATION_TEMPLATES: readonly RetailVisualizationTemplate[] = [
  ...
  },
  {
    ...
    density: 'dense',
    requiredComponents: ['经营 KPI 总览', '分日经营趋势', '商品经营阶段', '用户留存 cohort', '渠道与利润拆解', '库存异常', '行动建议', '合成口径与数据缺口说明'],
    optionalComponents: ['用户分群', '价格带对比'],
    panels: ['hero-panel', 'insight-strip:kpis', 'chart-zone:daily', 'chart-zone:lifecycle', 'chart-zone:retention', 'chart-zone:profit', 'chart-zone:inventory', 'action-strip', 'data-quality-footer'],
    firstViewport: ['hero-panel', 'insight-strip:kpis', 'chart-zone:daily'],
    variantGuidance: ['扩展分析接口返回的用户、渠道、成本和库存字段必须保留合成边界。', '商品阶段只解释窗口内购买活跃度，不得表述为真实上下架生命周期。', '价格弹性数据不足时展示缺口，不生成伪造系数。'],
    dataRequirements: ['datasets.biOverview', 'datasets.analyticsOverview', 'datasets.analyticsLifecycle', 'datasets.analyticsRetention', 'datasets.analyticsProfit', 'datasets.analyticsInventory'],
    dataSignals: ['kpis', 'daily', 'lifecycle', 'retention', 'channels', 'profit', 'inventory', 'actions', 'limitations'],
    painPoints: ['扩展经营分析数据集为演示数据，不能替代真实财务和供应链结论'],
    finalDataContract: ['window', 'plannedEntities', 'datasets.biOverview', 'datasets.analyticsOverview', 'datasets.analyticsLifecycle', 'datasets.analyticsRetention', 'datasets.analyticsProfit', 'datasets.analyticsInventory'],
  },
  {
