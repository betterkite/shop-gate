...
vi.mock('@/lib/agent/pi/run-engine', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@/lib/agent/pi/run-engine')>();
  return {
    ...actual,
    PiAgentRunEngine: class {
...
vi.mock('@/lib/services/pi-agent-prompts', () => ({
  ...
  assessPlatformPreparedArtifacts: mocks.assessPreparedArtifacts,
}));

vi.mock('@/lib/services/pi-agent-provenance', () => ({
  hashPiAgentProvenance: vi.fn(() => 'a'.repeat(64)),
  hashPiAgentWorkspace: vi.fn(async () => ({
    sha256: 'b'.repeat(64),
    fileCount: 1,
    hashedBytes: 1,
    metadataOnlyFiles: 0,
  })),
  hashPiAgentWorkspaceIdentity: vi.fn(async () => `sha256:${'c'.repeat(64)}`),
}));

vi.mock('@/lib/services/pi-agent-run-store', () => ({
  createPrismaPiAgentDurableRunSession: mocks.createDurableSession,
}));

vi.mock('@/lib/services/pi-agent-recovery', () => ({
...
vi.mock('@/lib/db/pi-agent-schema-readiness', () => ({
  PI_AGENT_SCHEMA_CONTRACT_VERSION: 'test-schema-contract',
  assertPiAgentSchemaReady: mocks.assertSchemaReady,
}));
...
describe('PI Agent terminal ownership', () => {
  ...
  it('keeps real conversation history while excluding automatic validation pipeline messages', async () => {
    ...
    mocks.getRecentMessages.mockResolvedValue([
      ...
      },
      {
        ...
        requestId: 'request-old',
        cliSource: 'pi',
        metadataJson: JSON.stringify({ isPiAgentFinal: true }),
      },
      {
        role: 'assistant',
  ...
  it('does not let a stricter cache-miss breaker shrink cumulative prepared input', async () => {
    ...
    try {
      ...
      delete process.env.PI_AGENT_MAX_RUN_CACHE_MISS_INPUT_TOKENS;
    }
  ...
  it('runs the trusted standard compiler without a model API key', async () => {
    ...
    expect(mocks.engineOptions).toHaveBeenCalledWith(expect.objectContaining({
      provider: expect.objectContaining({ name: 'pi-agent-trusted-renderer' }),
      model: 'pi-agent-deterministic-renderer-v1',
    }));
  });
  ...
  it('applies the total deadline to setup before the provider is contacted', async () => {
    process.env.PI_AGENT_TIMEOUT_MS = '20';
