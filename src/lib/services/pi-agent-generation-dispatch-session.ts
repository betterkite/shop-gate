...
import {
  ...
  type PiAgentGenerationDispatchClaim,
  type PiAgentGenerationDispatchFence,
...
export interface PiAgentGenerationDispatchSessionOptions {
  leaseOwner?: string;
  leaseTtlMs?: number;
...
const dispatchContext =
  new AsyncLocalStorage<PiAgentGenerationDispatchSession>();
 ...
 */
export class PiAgentGenerationDispatchSession {
  ...
  private constructor(
    readonly claim: PiAgentGenerationDispatchClaim,
    options: {
      leaseTtlMs: number;
      heartbeatIntervalMs: number;
      heartbeatEnabled: boolean;
    ...
    if (options.heartbeatEnabled) {
      this.timer = globalThis.setInterval(() => {
        ...
        void this.serializeHeartbeat(async () => {
          await heartbeatPiAgentGenerationJob({
            fence: this.fence,
  ...
  static async enqueueAndClaim(
    ...
    if (heartbeatEnabled && heartbeatIntervalMs >= leaseTtlMs) {
      throw new Error(
        "PI_AGENT_DISPATCH_HEARTBEAT_INTERVAL_MS must be smaller than the lease TTL.",
      );
    }
    await enqueuePiAgentGenerationJob(input);
    ...
    return new PiAgentGenerationDispatchSession(claim, {
      leaseTtlMs,
      heartbeatIntervalMs,
      heartbeatEnabled,
    });
  ...
  get fence(): PiAgentGenerationDispatchFence {
    return {
...
export function currentPiAgentGenerationDispatchFence():
  | PiAgentGenerationDispatchFence
  | undefined {
  return dispatchContext.getStore()?.fence;
}

export function currentPiAgentGenerationDispatchSession():
  | PiAgentGenerationDispatchSession
  | undefined {
  return dispatchContext.getStore();
}

export function assertCurrentPiAgentGenerationDispatchHealthy(): void {
