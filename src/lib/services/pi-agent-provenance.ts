...
const SNAPSHOT_VERSION = 'pi-agent-workspace-snapshot-v1';
const IDENTITY_VERSION = 'pi-agent-workspace-identity-v1';
...
export interface PiAgentWorkspaceSnapshot {
  sha256: string;
  fileCount: number;
  hashedBytes: number;
...
function canonicalJson(value: unknown, ancestors = new WeakSet<object>()): string {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw new Error('PI Agent provenance cannot hash non-finite numbers.');
    return JSON.stringify(value);
  }
  if (Array.isArray(value)) {
    if (ancestors.has(value)) throw new Error('PI Agent provenance cannot hash cyclic values.');
    ancestors.add(value);
    try {
  ...
  throw new Error(`PI Agent provenance cannot hash ${typeof value} values.`);
}

export function hashPiAgentProvenance(value: unknown): string {
...
export async function hashPiAgentWorkspace(
  workspaceRoot: string,
  options: {
    maxFiles?: number;
...
): Promise<PiAgentWorkspaceSnapshot> {
