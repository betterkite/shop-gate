...
export function retailAnalyticsBiPageTemplate(): string {
  ...
  return (
    <main className="dashboard-shell" data-visual-language="retail-workbench">
      ...
      <section className="chart-zone"><h2>价格实验模拟参考</h2>{experimentReady && experimentRows.length > 0 ? <table className="dense-table"><thead><tr><th>实验</th><th>对照价格</th><th>处理价格</th><th>对照购买率</th><th>处理购买率</th><th>购买率变化</th></tr></thead><tbody>{experimentRows.map((row, index) => <tr key={'experiment-' + index}><td>{text(row.experiment_id)}</td><td>{displayMoney(row.control_price)}</td><td>{displayMoney(row.treatment_price)}</td><td>{displayPercent(row.control_conversion_rate)}</td><td>{displayPercent(row.treatment_conversion_rate)}</td><td>{displayPercent(row.absolute_conversion_lift)}</td></tr>)}</tbody></table> : <p className="empty-state">{text(elasticity?.experiment_explanation, '当前数据没有同时包含对照组和处理组的价格实验记录，不能计算实验差异。')}</p>}<p className="footnote">这里只展示已登记实验或合成实验模拟的购买率差异；没有真实分组证据时，不把它解释为因果结论或直接调价建议。</p></section>
