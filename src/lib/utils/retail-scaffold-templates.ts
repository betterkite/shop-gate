...
export function retailAnalyticsBiPageTemplate(): string {
  ...
  return (
    <main className="dashboard-shell" data-visual-language="retail-workbench">
      ...
      <section className="chart-zone"><h2>价格实验模拟参考</h2>{experimentReady && experimentRows.length > 0 ? <table className="dense-table"><thead><tr><th>实验</th><th>对照价格</th><th>处理价格</th><th>对照购买率</th><th>处理购买率</th><th>购买率变化</th><th>变化范围（95%）</th><th>样本判断</th><th>分组证据</th></tr></thead><tbody>{experimentRows.map((row, index) => { const status = text(row.significance_status); const label = status === 'significant' ? '样本显示有差异' : status === 'small_sample' ? '样本较少，暂不判断' : '暂未发现明确差异'; const evidence = asRecord(row.assignment_evidence); const evidenceStatus = text(evidence?.status); const evidenceLabel = evidenceStatus === 'declared_randomized' ? (evidence?.synthetic ? '已记录分组（演示）' : '已记录分组') : evidenceStatus === 'not_verified' ? '未确认随机分组' : evidenceStatus === 'incomplete' ? '分组记录不完整' : '未提供用户分组'; return <tr key={'experiment-' + index}><td>{text(row.experiment_id)}</td><td>{displayMoney(row.control_price)}</td><td>{displayMoney(row.treatment_price)}</td><td>{displayPercent(row.control_conversion_rate)}</td><td>{displayPercent(row.treatment_conversion_rate)}</td><td>{displayPercent(row.absolute_conversion_lift)}</td><td>{displayPercent(row.absolute_conversion_lift_ci_low)} ～ {displayPercent(row.absolute_conversion_lift_ci_high)}</td><td>{label}<br /><small>p 值 {row.p_value == null ? '—' : text(row.p_value)}</small></td><td>{evidenceLabel}<br /><small>对照 {displayNumber(evidence?.control_assigned_users)} 人 / 处理 {displayNumber(evidence?.treatment_assigned_users)} 人</small></td></tr>; })}</tbody></table> : <p className="empty-state">{text(elasticity?.experiment_explanation, '当前数据没有同时包含对照组和处理组的价格实验记录，不能计算实验差异。')}</p>}<p className="footnote">变化范围表示按当前样本估算的 95% 区间；分组证据用于核对记录是否完整，不等于已经证明价格导致变化。合成实验尤其不能替代真实线上实验或直接调价依据。</p></section>
