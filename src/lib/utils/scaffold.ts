...
/**
 * 零售标准生成路径：按 run plan 的可视化 templateId（或 capabilityId）分发到
 * 能力看板模板，而不是落下仅含"数据集覆盖"的 base 骨架。
 *
 * 之前 `ensureQuantDashboardTemplate`（金融模板）识别不到零售 templateId 便提前
 * return，只保留 scaffoldBasicNextApp 写入的 base 页。此函数让标准看板直接产出
 * 对应能力的柱状图/趋势图/指标矩阵。
 */
